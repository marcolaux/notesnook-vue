...
exports.Tiptap = void 0;
const showdown_1 = __importDefault(require("@streetwriters/showdown"));
...
const html_rewriter_js_1 = require("../utils/html-rewriter.js");
const internal_link_js_1 = require("../utils/internal-link.js");
const dom_serializer_1 = require("dom-serializer");
const logger_js_1 = require("../logger.js");
const ExtractableTypes = ["blocks", "internalLinks", "blocksWithLink"];
const ATTRIBUTES = {
    hash: "data-hash",
    mime: "data-mime",
...
showdown_1.default.helper.document = (0, html_parser_js_1.getDummyDocument)();
...
class Tiptap {
    ...
    toHeadline() {
        return (0, html_parser_js_1.extractHeadline)(this.data, 350);
    }
    toTitle() {
        return (0, html_parser_js_1.extractTitle)(this.data, 60);
    }
    // isEmpty() {
    ...
    insertBlockIds() {
        let index = 0;
        return new html_rewriter_js_1.HTMLRewriter({
            ontag(name, attr) {
                switch (name) {
                    case "p":
                    case "h1":
                    ...
                    case "div":
                        return {
                            name,
                            attr: Object.assign(Object.assign({}, attr), { [ATTRIBUTES.blockId]: `${name}${++index}` })
                        };
                }
            }
        }).transform(this.data);
    ...
    insertMedia(resolve) {
        return __awaiter(this, void 0, void 0, function* () {
            const hashes = [];
            new html_parser_js_1.HTMLParser({
                ontag: (name, attr) => {
    ...
    resolveAttachments(resolve) {
        return __awaiter(this, void 0, void 0, function* () {
            const attachments = {};
            const document = (0, htmlparser2_1.parseDocument)(this.data);
            const elements = (0, domutils_1.findAll)((e) => !!e.attribs[ATTRIBUTES.hash], document.childNodes);
            elements.forEach((element) => (attachments[element.attribs[ATTRIBUTES.hash]] = element.attribs));
            ...
            for (const element of elements) {
                ...
                (0, domutils_1.replaceElement)(element, (0, htmlparser2_1.parseDocument)(html));
            }
            this.data = (0, dom_serializer_1.render)(document);
            return this;
        });
    }
    resolveInternalLinks(resolve) {
        this.data = new html_rewriter_js_1.HTMLRewriter({
            ontag: (name, attr) => {
    ...
    extract(...types) {
        ...
        if (types.includes("blocksWithLink")) {
            result.blocks.push(...(0, domutils_1.findAll)((element) => {
                return (!!element.attribs[ATTRIBUTES.blockId] &&
                    element.childNodes.some((node) => {
                        var _a;
                        return (0, domutils_1.isTag)(node) &&
                            node.tagName === "a" &&
                            (0, internal_link_js_1.isNoteLink)(node.attribs.href) &&
                            ((_a = (0, internal_link_js_1.parseInternalLink)(node.attribs.href)) === null || _a === void 0 ? void 0 : _a.type) === "note";
                    }));
            }, document.childNodes).map((element) => {
                return {
                    id: element.attribs[ATTRIBUTES.blockId],
                    type: element.tagName.toLowerCase(),
                    content: convertHtmlToTxt(this.data.slice(element.startIndex || 0, element.endIndex || 0), false)
                };
            }));
        }
        if (types.includes("blocks")) {
            result.blocks.push(...(0, domutils_1.findAll)((element) => {
                return (0, domutils_1.isTag)(element) && !!element.attribs[ATTRIBUTES.blockId];
            }, document.childNodes).map((node) => ({
                id: node.attribs[ATTRIBUTES.blockId],
                type: node.tagName.toLowerCase(),
                content: convertHtmlToTxt(this.data.slice(node.startIndex || 0, node.endIndex || 0), false)
            })));
        }
        if (types.includes("internalLinks")) {
            result.internalLinks.push(...(0, domutils_1.findAll)((e) => e.tagName === "a" && !!e.attribs.href && (0, internal_link_js_1.isNoteLink)(e.attribs.href), document.childNodes)
                .map((e) => (0, internal_link_js_1.parseInternalLink)(e.attribs.href))
                .filter((v) => !!v));
        }
        return result;
    }
    ...
    removeAttachments(hashes) {
        return new html_rewriter_js_1.HTMLRewriter({
            ontag: (_name, attr) => {
                if (hashes.includes(attr[ATTRIBUTES.hash]))
                    return false;
            }
        }).transform(this.data);
    }
    postProcess(saveAttachment) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!this.data.includes(ATTRIBUTES.src) &&
            ...
            new html_parser_js_1.HTMLParser({
                ontag: (name, attr, pos) => {
                    ...
                    if (name === "img" && !hash && src) {
                        ...
                        const internalLink = (0, internal_link_js_1.parseInternalLink)(href);
                        if (!internalLink)
                            return;
            ...
            for (const image of sources) {
                try {
                    const { data, mimeType } = dataurl_js_1.default.toObject(image.src);
                    if (!data || !mimeType)
                        continue;
                    ...
                    logger_js_1.logger.error(e, "Failed to save image attachment.", {
            ...
            const hashes = [];
            const html = new html_rewriter_js_1.HTMLRewriter({
                ontag: (name, attr, pos) => {
                    switch (name) {
                ...
                if (elem.parent && elem.parent.name === "li") {
                    ...
                    if (elem.children && elem.children.length > 0) {
                        ...
                        if (firstChild.type === "text" && firstChild.data) {
                            firstChild.data = firstChild.data.replace(/^([ \t]+)/, (match) => {
                                return match
                                    .replace(/ /g, "\u00A0")
                                    .replace(/\t/g, "\u00A0\u00A0\u00A0\u00A0");
                            });
...
function formatList(elem, walk, builder, formatOptions, nextPrefixCallback) {
    var _a;
    const isNestedList = ((_a = elem === null || elem === void 0 ? void 0 : elem.parent) === null || _a === void 0 ? void 0 : _a.name) === "li";
    // With Roman numbers, index length is not as straightforward as with Arabic numbers or letters,
    // so the dumb length comparison is the most robust way to get the correct value.
    let maxPrefixLength = 0;
    const listItems = (elem.children || [])
        // it might be more accurate to check only for html spaces here, but no significant benefit
        .filter((child) => child.type !== "text" || (child.data && !/^\s*$/.test(child.data)))
        .map(function (child) {
        if (child.name !== "li") {
