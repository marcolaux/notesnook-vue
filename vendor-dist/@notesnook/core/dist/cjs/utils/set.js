...
class SetManipulator {
    ...
    process(a, b, key = (item) => String(item), evaluator) {
        ...
        b.forEach((value) => {
            ukey = key(value);
            if (hist[ukey]) {
                if (hist[ukey].frequency === 1)
                    hist[ukey].frequency = 3;
            }
            else
                hist[ukey] = { value: value, frequency: 2 };
        });
        ...
        if (evaluator) {
            for (const key in hist) {
                ...
                if (evaluator(hist[key].frequency))
                    out.push(hist[key].value);
            }
            return out;
        }
        return hist;
...
exports.set = new SetManipulator();
