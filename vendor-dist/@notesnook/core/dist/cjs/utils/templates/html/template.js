...
<html lang="en-US">
  <head>
    ...
    <meta
      name="description"
      content="${(0, entities_1.escapeUTF8)(data.headline || "")}"
    />
    <title>${(0, entities_1.escapeUTF8)(data.title)}</title>
    <meta name="created-at" content="${(0, date_js_1.formatDate)(data.dateCreated)}" />
    <meta name="updated-at" content="${(0, date_js_1.formatDate)(data.dateEdited)}" />
    ${data.pinned ? `<meta name="pinned" content="${data.pinned}" />` : ""}
    ${data.favorite ? `<meta name="favorite" content="${data.favorite}" />` : ""}
    ${data.color
        ? `<meta name="color" content="${(0, entities_1.escapeUTF8)(data.color)}" />`
        : ""}
    ${data.tags && data.tags.length
        ? `<meta name="tags" content="${(0, entities_1.escapeUTF8)(data.tags.join(", "))}" />`
        : ""}
  ...
  <body>
    <h1>${(0, entities_1.escapeUTF8)(data.title)}</h1>
    ${data.content}
  </body>
</html>
`;
