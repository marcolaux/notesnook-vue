...
class FileStorage {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (this.groups.downloads.has(groupId)) {
                    logger_js_1.logger.debug("[queueDownloads] group already exists", {
                        groupId
                    });
                ...
                for (const file of files) {
                    ...
                    if (!group.has(file.filename)) {
                        this.eventManager.publish(common_js_1.EVENTS.fileDownloaded, {
                            success: false,
                            groupId,
                    ...
                    if (download && download.operation) {
                        logger_js_1.logger.debug("[queueDownloads] duplicate download", {
                            filename: file.filename,
                            groupId
                        });
                    ...
                    this.eventManager.publish(common_js_1.EVENTS.fileDownload, {
                        total,
                        current,
                        groupId,
                        filename
                    });
                    const url = `${constants_js_1.default.API_HOST}/s3?name=${filename}`;
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (this.groups.uploads.has(groupId)) {
                    logger_js_1.logger.debug("[queueUploads] group already exists", {
                        groupId
                    });
                ...
                for (const file of files) {
                    ...
                    if (upload && upload.operation) {
                        logger_js_1.logger.debug("[queueUploads] duplicate upload", {
                            filename: file.filename,
                            groupId
                        });
                    ...
                    file.operation = execute()
                        .catch((e) => {
                        logger_js_1.logger.error(e, "failed to upload attachment", { hash: filename });
                        error = e;
                        return false;
                    })
                        .finally(() => {
                    ...
                    this.eventManager.publish(common_js_1.EVENTS.fileUpload, {
                        total,
                        current,
                        groupId,
                        filename
                    ...
                    const result = yield file.operation;
                    this.eventManager.publish(common_js_1.EVENTS.fileUploaded, {
                        error,
                        success: result,
                        total,
                        current: ++current,
    ...
    downloadFile(groupId, filename, chunkSize) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (download && download.operation) {
                logger_js_1.logger.debug("[downloadFile] duplicate download", { filename, groupId });
                return yield download.operation;
            }
            logger_js_1.logger.debug("[downloadFile] downloading", { filename, groupId });
            const url = `${constants_js_1.default.API_HOST}/s3?name=${filename}`;
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const queue of queues) {
                ...
                if (queue.type === "download") {
                    this.groups.downloads.delete(groupId);
                    this.eventManager.publish(common_js_1.EVENTS.downloadCanceled, {
                        groupId,
                        canceled: true
                    });
                }
                    ...
                    this.groups.uploads.delete(groupId);
                    this.eventManager.publish(common_js_1.EVENTS.uploadCanceled, {
                        groupId,
                        canceled: true
                    });
                }
    ...
    bulkDeleteFiles(filenames_1) {
        return __awaiter(this, arguments, void 0, function* (filenames, localOnly = false) {
            ...
            const token = yield this.tokenManager.getAccessToken();
            const url = `${constants_js_1.default.API_HOST}/s3/bulk-delete`;
            return yield this.fs.bulkDeleteFiles(filenames, {
                url,
                headers: { Authorization: `Bearer ${token}` },
                chunkSize: 0
