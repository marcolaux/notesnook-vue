...
export const parseHTML = (input) => "DOMParser" in globalThis
...
export function normalizeToHtmlBody(input) {
    var _a, _b, _c;
...
export function extractHeadline(html, characterLimit) {
    let text = "";
    let start = false;
    const parser = new Parser({
        onopentag: (name) => {
...
export function extractTitle(html, characterLimit) {
    let text = "";
...
export function extractMatchingBlocks(html, matchTagName) {
    const matches = [];
    let text = "";
