...
*/
import { parseInternalLink } from "./internal-link.js";
const INTERNAL_LINK_REGEX = /(?:\[\[(nn:\/\/note\/.+?)\]\])/gm;
export function extractInternalLinks(block) {
    ...
    for (const match of matches) {
        ...
        if (!link)
            continue;
        links.push(Object.assign(Object.assign({}, link), { start: match.index, end: match.index + match[0].length, text }));
    }
    return links;
}
...
export function highlightInternalLinks(block, noteId) {
...
export function ellipsize(text, maxLength, from) {
