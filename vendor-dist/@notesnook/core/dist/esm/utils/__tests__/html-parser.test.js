...
describe("normalizeToHtmlBody", () => {
    ...
    it("should handle runtime non-string values safely", () => {
        expect(normalizeToHtmlBody(null)).toBe("<html><body></body></html>");
        expect(normalizeToHtmlBody(undefined)).toBe("<html><body></body></html>");
    });
});
...
describe("sanitizeHtml", () => {
    ...
    it("strips javascript: URIs from src", () => {
        // eslint-disable-next-line no-script-url
        const result = sanitizeHtml('<iframe src="javascript:alert(document.domain)"></iframe>');
        expect(result).not.toContain("javascript:");
    });
    it("strips onclick and other on* attributes", () => {
        const result = sanitizeHtml('<button onclick="evil()">OK</button><div onmouseover="evil()">x</div>');
        expect(result).not.toContain("onclick");
        expect(result).not.toContain("onmouseover");
        expect(result).not.toContain("evil()");
    });
    it("strips <object> and <embed> tags", () => {
        const result = sanitizeHtml('<object data="malicious.swf"></object><embed src="evil.swf">');
    ...
    it("handles deeply nested XSS attempts", () => {
        const result = sanitizeHtml("<div><p><span onmouseover=\"alert('xss')\">hover</span></p></div>");
        expect(result).not.toContain("onmouseover");
        expect(result).toContain("hover");
    });
    it("strips <base> tag that could hijack relative URLs", () => {
        const result = sanitizeHtml('<base href="https://evil.com"><a href="/path">link</a>');
    ...
    it("strips event handlers from <iframe>", () => {
        const result = sanitizeHtml('<iframe src="https://example.com" onload="steal()"></iframe>');
        expect(result).toContain("<iframe");
        expect(result).not.toContain("onload");
        expect(result).not.toContain("steal()");
    });
    it("preserves nested <iframe> with safe src alongside other elements", () => {
        const result = sanitizeHtml('<div><p>Safe content</p><iframe src="https://example.com"></iframe></div>');
