...
export default class Lookup {
    ...
    notes(query, notes) {
        return this.toSearchResults((limit, sortOptions) => __awaiter(this, void 0, void 0, function* () {
            ...
            const ftsResults = (yield ((_a = this.ftsQueryBuilder({ content: content === null || content === void 0 ? void 0 : content.query, title: title === null || title === void 0 ? void 0 : title.query }, excludedIds, notes)) === null || _a === void 0 ? void 0 : _a.select(["results.id"]).groupBy("results.id").orderBy(sql `SUM(results.rank)`, (sortOptions === null || sortOptions === void 0 ? void 0 : sortOptions.sortDirection) || "desc").execute().catch((e) => {
    ...
    notesWithHighlighting(query, notes, sortOptions) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const excludedIds = this.db.trash.cache.notes;
            const { content, title, tag, color, archived, favorite, locked, pinned, readonly, created_before, created_after, edited_after, edited_before, colored, tagged, in_notebook, filters } = transformQuery(query);
            if (filters > 0) {
                const tagIds = tagged
            ...
            const regexMatches = (yield ((_b = this.regexQueryBuilder({
                content: filterSmallTokens(content === null || content === void 0 ? void 0 : content.tokens),
                title: filterSmallTokens(title === null || title === void 0 ? void 0 : title.tokens)
            }, (!!content || !!title) && ftsIds.length > 0 ? ftsIds : notes)) === null || _b === void 0 ? void 0 : _b.select(["results.id", "results.type", sql `1`.as("rank")]).execute())) || [];
            ...
            return new VirtualizedGrouping(matches.ids.length, 20, () => __awaiter(this, void 0, void 0, function* () { return matches.ids; }), (start, end) => __awaiter(this, void 0, void 0, function* () {
                ...
                for (const html of htmls) {
                    ...
                    result.content = extractMatchingBlocks(highlighted, MATCH_TAG_NAME).flatMap((block) => {
    ...
    ftsQueryBuilder(queries, excludedIds = [], filter) {
        ...
        function buildTitleQuery(eb) {
            return eb
                ...
                .where("rank", "=", sql `'bm25(1.0, 10.0)'`)
                .select(["id", "rank", sql `'title'`.as("type")]);
        }
        function buildContentQuery(eb) {
            return eb
                ...
                .where("rank", "=", sql `'bm25(1.0, 1.0, 10.0)'`)
                .select(["noteId as id", "rank", sql `'content'`.as("type")])
                .$castTo();
        }
        if (queries.content && queries.title)
    ...
    regexQueryBuilder(queries, ids) {
        ...
        function buildTitleQuery(eb, queries) {
            ...
            return eb
                ...
                .where("title", "regexp", sql `${regex}`)
                .select(["id", sql `'title'`.as("type")]);
        }
        function buildContentQuery(eb, queries) {
            ...
            return eb
                ...
                .where("data", "regexp", sql `${regex}`)
                .select(["noteId as id", sql `'content'`.as("type")])
                .$castTo();
        }
        if (queries.content && queries.title)
    ...
    filter(selector_1, query_1, fields_1) {
        return __awaiter(this, arguments, void 0, function* (selector, query, fields, options = {}) {
            ...
            selector.fields([]);
            return fuzzy(query, items, (item) => item.id, Object.fromEntries(fields.filter((f) => !f.ignore).map((f) => [f.name, f.weight || 1])), options);
        });
    }
...
export function splitHighlightedMatch(text) {
