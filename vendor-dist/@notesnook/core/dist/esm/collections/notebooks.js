...
import { isTrashItem } from "../types.js";
...
export function withSubNotebooks(db, ids, excluded) {
    return db.withRecursive(`subNotebooks(id, path, rootId)`, (eb) => eb
        .selectFrom(() => sql `(VALUES ${sql.join(ids.map((id) => sql.raw(`('${id}', '${id}', '${id}')`)))})`.as("roots"))
        .selectAll()
        .unionAll((eb) => eb
        .selectFrom(["relations", "subNotebooks"])
        .select([
        ...
        sql `subNotebooks.path || '/' || relations.toId`.as("path"),
        ...
        .where("subNotebooks.path", "not like", sql `'%' || relations.toId || '%'`)
        .$narrowType()));
}
