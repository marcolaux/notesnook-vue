 ...
 *
 * Labels are English literals (the codebase is mid-i18n — ArchiveView / TrashView
 * hardcode the same way; migrating these is the Phase 7.1 sweep). The public URL
...
async function unpublishItem(item: MonographsListItem): Promise<void> {
  const ok = await dialog.confirm({
    ...
    message: `Unpublish “${item.title}”? It will no longer be public and the link will stop working.`,
...
function onRowContext(item: MonographsListItem, e: MouseEvent): void {
  const items: MenuItem[] = [
    { id: "copy-url", label: "Copy URL", onSelect: () => copyUrl(item) },
    { id: "open", label: "Open in browser", onSelect: () => openInBrowser(item) },
    separator("sep"),
    {
      id: "unpublish",
      label: "Unpublish",
...
<template>
  <div class="flex min-h-0 min-w-0 flex-1 flex-col backdrop-blur-xl">
    ...
    <div class="flex h-9 shrink-0 items-center gap-2 border-b border-glass-border px-3">
      <span class="text-xs font-semibold text-text">Monographs</span>
      <span class="text-[10px] text-text-muted">{{ monographs.count }} published</span>
    </div>
      ...
      >
        <div class="flex items-center gap-1">
          ...
          <span
            ...
            title="Self-destructs after first view"
          >1×</span>
        ...
        <div class="mt-0.5 flex items-center gap-1.5 text-[9px] text-text-muted">
          <span v-if="item.totalViews !== undefined" title="Total views">{{ item.totalViews }} views</span>
          <span v-if="item.publishUrl" class="truncate">{{ item.publishUrl }}</span>
          <span class="ml-auto shrink-0">Published {{ formatDate(item.datePublished) }}</span>
        </div>
      </button>
      <div v-if="!monographs.loading && monographs.items.length === 0" class="px-2 py-4 text-center text-[10px] text-text-muted">
        No published notes
