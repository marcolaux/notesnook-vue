...
- [~] **2.6 `@notesnook/intl`-Quelle klären** — Lingui-Strings aus Hauptrepo
  ...
  - **Status 2026-07-20 (vue-i18n-Foundation gelandet, volle Migration = 7.1):**
    Entscheidung #1 (Lingui vs. vue-i18n) **pro vue-i18n** getroffen. Begründung:
    Upstream `@notesnook/intl` ist Lingui+`@lingui/macro`+React-gekoppelt (2810-Zeilen
    `strings.ts` importiert ein `generated/`-Codegen aus der Upstream-React-App;
    `react` als peerDep) und liefert nur `en` + eine Pseudo-Locale — keine echten
    Übersetzungen zu porten. Unser Vue-Renderer spiegelt die Upstream-React-
    Komponentenstruktur nicht 1:1, also würde der Upstream-String-Katalog ohnehin
    nicht sauber mappen. Ein Vue-nativer Katalog mit eigenen Keys ist einfacher,
    hat keinen Macro-Compile-Schritt + kein React-peerDep. (Upstream `core`
    bundlet `intl` ohnehin in sein dist — der Renderer konsumiert `@notesnook/intl`
    gar nicht.)
  - Neues `apps/desktop/src/renderer/src/i18n/`:
    - `index.ts` — `createI18n({legacy:false, locale: readLocale(), fallbackLocale:
      "en", messages:{en,pseudo}})` + `setLocale(locale)` (switcht
      `i18n.global.locale.value` + persistiert nach `localStorage
      "notesnook.locale"`, best-effort try/catch wie settings-`themeMode`) +
      `LOCALES`/`Locale`/`PSEUDO_LOCALE`-Konstanten.
    - `locales/en.ts` — eigener English-Katalog (nested by feature), seeded mit
      `sidebar.notebooks`/`noNotebooks`/`tags`/`noTags` + `common.greeting`.
    - `locales/pseudo.ts` — `toPseudo(messages)` wrappt jedes String-Leaf in
      `⟪…⟫` (Guillemets) → Pseudo-Locale ist ein Voll-Copy von en (App bleibt
      nutzbar), unveröffentlichte Strings werden sofort sichtbar. Generiert sich
      aus `en` → driftet nicht.
  - `main.ts` — `app.use(i18n)` (Composition-API-Modus → `$t`/`useI18n`).
  - **Slice (Proof-of-Pattern):** `Sidebar.vue` `useI18n()` + 4 Strings →
    `t("sidebar.*")` (Notebooks/No notebooks/Tags/No tags). Bewusst keine
    Komponente mit existierendem Mount-Test gewählt (Sidebar hat keinen → kein
    Test braucht das i18n-Plugin installiert).
  - `vue-i18n@^11.4.6` als devDep (renderer-only, Vite-inline-bundled → nicht
    von electron-builder gepackt, nicht von `externalizeDepsPlugin` externalisiert).
  - 16 neue Contract-Tests in `i18n.spec.ts` (happy-dom): `toPseudo` rein,
    Locale-Konstanten, Instance (en-Katalog, pseudo wrappt, Fallback auf en für
    ungeladene Locale + fehlende Keys, total-safe), `setLocale` (switcht +
    persistiert + never-throws ohne localStorage), Initial-Read via
    `vi.resetModules`+dyn. Import (persisted pseudo→pseudo, none→en, invalid→en),
    Komponenten-Integration (`useI18n().t` rendert + reagiert reaktiv auf
    `setLocale`).
  - **Verifiziert (headless):** 578 Contract-Tests grün (562 + 16) über 40
    Spec-Dateien; typecheck (node+web+contracts) + `electron-vite build` clean
    (renderer +139 KB für inline-bundletes vue-i18n, kein React/theme-ui/zustand-
    Leck).
  - **Aufgeschoben (Phase 7.1 Polish):** volle String-Migration (Dutzende
    hardcoded English-Strings in stores/utils/components — z.B. `syncStatusText`,
    `updateStatusText`, `VAULT_ERROR_MESSAGES`, Palette-Command-Titles,
    NotesList/StatusBar/LoginScreen-Texte), Locale-Switcher-UI in SettingsView,
    weitere Locales (echte Übersetzungen), Entscheidung ob stores/utils Codes
    emitieren + Views übersetzen (clean) oder eine globale `t`-Instanz importieren
    (einfacher). Pseudo-Locale über einen Dev-Switch/Flag aktivierbar machen.
...
- [~] **5.1 Properties-Panel als rechtes Sidebar** — Title, Notebook, Tags,
  ...
  - **Status 2026-07-20 (Teil 2 — Tags/Notebooks-Zuweisung, headless):** das
    Properties-Backend bekommt die aufgeschobenen Tag- + Notebook-Zuweisungen
    der aktiven Note. `utils/properties.ts` neu: `AssignedTag`/`AssignedNotebook`
    (`{id,title}`), `toAssignedTag`/`toAssignedNotebook` (Untitled-Fallback)
    + `uniqueById` (defensiv gg. Dubletten aus `relations.resolve`).
    `stores/properties.ts` erweitert: `tags`/`notebooks`-Refs +
    `loadingAssignments`/`busy`/`lastError` + `loadAssignments` (liest
    `db.relations.to({id,type:"note"},"tag"|"notebook").resolve()` parallel,
    never-throws, Reset bei keiner aktiven Note) + `addTag`/`removeTag`
    (`db.relations.add`/`unlink` mit tag→note-Richtung, Upstream-Konvention) +
    `createTag` (datenrückgebend: `db.tags.add({title})`→id + link, returned
    `AssignedTag|null`) + `addNotebook`/`removeNotebook`
    (`db.notes.addToNotebook`/`removeFromNotebook`); alle mutierenden
    never-throws, `busy`-gate, `lastError` bei Throw, je `loadAssignments` +
    `notes.load()` danach. `activeNoteId`-Watch ruft `loadAssignments` mit auf.
    13 neue Contract-Tests in `properties.spec.ts` (mappers, uniqueById,
    loadAssignments parallel, add/remove/create tag, add/remove notebook,
    never-throws-error, no-active-note). **493 Contract-Tests grün (480 + 13)**
    über 35 Spec-Dateien, typecheck (node+web+contracts) + build clean, 0
    React/theme-ui/zustand-Leck.
...
- [~] **6.1 Sync-Status** — `database.sync`-Events abonnieren, Sidebar-Account-Anzeige
  ...
  - **Status 2026-07-20 (Display-Done in 3.4, Control-Store headless):** die
    ...
    `hasUnsyncedChanges` + tickende relative Zeit; `StatusBar.vue` zeigt es).
    Neu (dieses Inkrement): der Sync-*Control*-Store als Komplement.
    `utils/sync.ts` (pure `buildSyncOptions` default `type:"full"`, setzt
    `force`/`offlineMode` nur wenn definiert — `exactOptionalPropertyTypes`-
    safe; `SYNC_TYPE_LABELS`) + `stores/sync.ts` (`useSyncStore`: `busy`/
    `lastError`/`lastResult`; `startSync`→`db.sync(opts)` boolean, `stopSync`→
    `db.syncer.stop()`, `cancelSync`→`db.syncer.sync.cancel()`; alle
    never-throws). **Bewusst separat vom Status-Store**: Status = Event-getriebene
    *Anzeige* (auch Auto-Sync feuert Events ohne `startSync`); Control =
    `busy`/`lastResult` eines `startSync`-Aufrufs (Button-Level). Kein
    Event-Subscribe hier → isoliert testbar. `SyncOptions` re-exportiert aus
    Contracts. `CommandContext`+`sync`-Feld; Palette-Command `app:sync-now`
    (`when: showShell` → `ctx.sync.startSync({type:"full"})`). 18 neue
    Contract-Tests in `sync.spec.ts` (pure `buildSyncOptions`/`SYNC_TYPE_LABELS`
...
- [~] **6.6 Spell-Checker** — Electron `session.defaultSession.spellcheck`
  - **Status 2026-07-20 (headless store + main impl, UI on-site):** das
    ...
    dep-frei, von Main + Renderer + Tests geteilt): `Language`-Typ +
    `LANGUAGES`-Tabelle (60+ Codes, verbatim vom Upstream
    `apps/desktop/src/api/spell-checker.ts`) + `LANGUAGE_REDIRECT_MAP`
    (`es`→`es-MX` etc.) + `resolveLanguage`/`resolveEnabledCodes` (Code→
    verfügbar-Set, Redirect-Map, Bare-Tag-Fallback, `undefined` wenn nichts
    passt) + `languageName`/`toLanguage`/`sortLanguages` +
    `SPELLCHECKER_ENABLED_DEFAULT=true`. `contracts/router.ts`: neuer
    `SpellCheckerServer` (`isEnabled`/`languages`/`enabledLanguages`/
    `setLanguages`/`toggle`/`words`/`deleteWord`) + `registerSpellCheckerServer`
    + `spellChecker`-tRPC-Router (zod-Inputs `z.array(z.string())` +
    `z.object({enabled})` + `z.string()`). `main/spell-checker.ts`: impl
    gebunden an `window.webContents.session` (Fallback
    `session.defaultSession`), persistiert das `enabled`-Flag nach
    `spellchecker.json` in userData (mirror `safe-storage.ts`), wendet es beim
    Register an; `main/index.ts` ruft `registerSpellChecker(window)` nach
    `createMainWindow`. Renderer `stores/spell-checker.ts`
    (`useSpellCheckerStore`): `busy`/`enabled`/`availableLanguages`/
    `enabledLanguages`/`dictionaryWords`/`lastError` + `enabledCodes`-Computed;
    `refresh` (lädt alle 4 parallel), `toggleSpellCheck` (applied returned
    state), `setLanguages` (re-reads enabledLanguages), `deleteWord` (entfernt
    lokal) — alle never-throws über `desktop.spellChecker.*`-Bridge. Kein
    Event-Subscribe (Session-Spellcheck feuert keine Lifecycle-Events →
    isoliert testbar, request/response wie `sync`-Control-Store).
    `CommandContext.spellChecker` (viral — 5 ctx-stub-Specs aktualisiert) +
    Palette-Command `app:toggle-spell-check` (`when: showShell`, toggelt
    `!enabled`). `App.vue` `refresh` im Boot + showShell-Watch. 26 neue
    Contract-Tests in `spell-checker.spec.ts` (pure Helper + Store mit
    `vi.hoisted`-gemockter `desktop-bridge` + Palette-Cmd).
    **562 Contract-Tests grün (536 + 26)** über 39 Spec-Dateien, typecheck
    (node+web+contracts) + build clean, 0 React/theme-ui/zustand-Leck
    (`electron` session-API nur im Main-Bundle).
    **Aufgeschoben (on-site):** Spell-Checker-UI (Enable-Toggle +
    Sprach-Picker für available/enabled + Custom-Dictionary-Liste mit
    Delete-Wort in SettingsView), Live-Verifikation (Tippfehler-Underline im
    Editor, Rechtschreib-Kontextmenü).
    **On-Site-Gate:** (nach UI) Toggle an → Tippfehler werden im Editor
    underlined; Sprachen wählen → Underline folgt der Sprache; Wort aus
    Dictionary löschen → nicht mehr ignoriert.
...
- [~] **7.5 electron-builder** — dmg + zip (macOS arm64/x64), nsis + portable
  ...
  - **Status 2026-07-20 (config + `--dir`-Build verifiziert, installer/sign/icons
    on-site/release):** das Packaging-Setup steht. Neues
    `apps/desktop/electron-builder.yml` (adaptiert vom Upstream
    `electron-builder.config.js` auf diesen Stack): `appId
    org.notesnookvue.desktop` + `productName "Notesnook Vue"` (env-overridable),
    `asar:true` + `asarUnpack: better-sqlite3-multiple-ciphers` (einziges native
    Modul — M4 droppte FTS5-Extensions, `trigram` ist SQLite-Built-in; kein
    `sqlite-better-trigram`/`sqlite3-fts5-html`; Sodium = vendorter JS/WASM,
    keine `sodium-native`-Prebuilds), `files: [out/**, package.json]`,
    `protocols: nn://` (Phase 6.5), mac `dmg`+`zip` arm64/x64, win `nsis`+`portable`
    x64, linux `AppImage`+`snap` x64, `publish: github` (treibt den Phase-6.2-
    Updater — electron-builder generiert `app-update.yml` automatisch, kein
    hand-gepflegtes nötig). Scripts `package`/`package:dir`/`package:mac`/
    `package:win`/`package:linux` in `apps/desktop/package.json`.
    **Zwei Fixes beim Verifizieren:** (1) `electron` war caret-gepinnt
    (`^37.10.3`) → electron-builder kann keine Version aus einem Range berechnen
    → auf exact `37.10.3` gepinnt (+ Lockfile via `npm install
    --package-lock-only` reconciled, damit `npm ci` in CI grün bleibt). (2)
    **Workspace-Dep-Split:** electron-builder kann keine Workspace-Packages
    packen deren Realpfad außerhalb des App-Dirs liegt (`packages/*` + die
    vendored `vendor-dist/@notesnook/*` — Fehler "must be under apps/desktop").
    Da Main/Preload/Contracts **keine** `@notesnook/*`- oder `@notesnook-vue/*`-
    Packages importieren (nur der Renderer, der sie via Vite inline-bundled),
    wurden alle renderer-only Deps (`@notesnook-vue/*`, `@notesnook/*`,
    `@tiptap/*`, `pinia`, `vue`, `vue-router`, `@trpc/client`) von
    `dependencies` → `devDependencies` verschoben. In `dependencies` bleiben
    nur die 6 Main-Runtime-Deps die `externalizeDepsPlugin` externalisiert +
    electron-builder packt: `better-sqlite3-multiple-ciphers`, `electron-updater`,
    `electron-trpc`, `@trpc/server`, `zod`, `@streetwriters/kysely`.
    **Verifiziert (headless):** `npm run package:dir` baut ein unsigned
    `Notesnook Vue.app` (mac arm64) — native Module für Electron-ABI rebuilt,
    `app.asar` (14 MB) enthält `out/main`+`out/preload`+`out/renderer` + nur die
    6 Runtime-Deps (+ transitive); `better_sqlite3.node` korrekt asar-unpacked;
    **0 renderer-only Bloat** im asar (kein `@notesnook`/`@tiptap`/`vue`/`pinia`/
    `@trpc/client`/`@notesnook-vue` geleaked). typecheck (node+web+contracts) +
    562 Contract-Tests + `electron-vite build` clean nach dem Dep-Split.
    **Aufgeschoben (on-site/release):** echte Installer (`dmg`/`nsis`/`AppImage`/
    `snap` — brauchen je Target-Host + z.T. Netz für snap), branded Icons
    (`build/icon.{icns,ico,png}` — fehlen, electron-builder nutzt Default-Icon +
    warnt), Code-Signing/Notarize (keine Certs — `--publish never` baut unsigned,
    fine für dev; `hardenedRuntime`/`entitlements`/`notarize`/`signtool` für
    Distribution), Cross-Arch-Builds (mac x64, win/linux arm64 brauchen die
    jew. native-Modul-Prebuilds/Toolchain), Packaging-Step in CI (7.6 baut nur
    typecheck+test+build, nicht package — bewusst; release pipeline = Folge),
    `app-update.yml`-Live-Test (Phase 6.2 on-site).
    **On-Site-Gate:** `npm run package:mac` → `Notesnook Vue.app` launchen →
    bootet end-to-end (dieselben Gates wie `npm run dev`); `npm run package`
    produziert Installer.
...
|---|---|---|---|
| 1 | **Lingui vs. vue-i18n** ✅ gelöst | `@lingui/react` ist React-spezifisch (`<Trans>`). `@notesnook/intl` nicht auf npm. | **Gelöst 2026-07-20 (2.6): vue-i18n.** Upstream `@notesnook/intl` ist Lingui+`@lingui/macro`+React-gekoppelt (2810-Zeilen `strings.ts` importiert ein `generated/`-Codegen aus der Upstream-React-App; `react` peerDep) + liefert nur `en`/pseudo (keine echten Übersetzungen). `core` bundlet `intl` in sein dist → der Renderer konsumiert es gar nicht. Unser Vue-Renderer spiegelt die Upstream-React-Struktur nicht 1:1 → Vue-nativer Katalog mit eigenen Keys (`vue-i18n@^11`, Composition-API, kein Macro-Compile, kein React-peerDep). Pseudo-Locale via `toPseudo`-Generator aus `en`. Volle String-Migration = Phase 7.1. |
...
## Phase 6.1 — Sync-Control-Store (headless) (2026-07-20)

Drittes Phase-6-Inkrement (nach 6.3 Vault + 6.7 Backup). Die Sync-Status-
*Anzeige* stand seit Phase 3.4 (`useStatusStore` subscribed `syncProgress`/
`syncCompleted`/`syncAborted` + `lastSynced` + `hasUnsyncedChanges`). Dieses
Inkrement fügt den Sync-*Control*-Store hinzu — das Komplement zum Anzeige-
Store.

**API geklärt** (`vendor-dist/@notesnook/core/dist/types/api/index.d.ts` +
`api/sync/index.d.ts`):
- `db.sync(options: SyncOptions): Promise<boolean>` — public Start (delegiert
  an `db.syncer.start`). `SyncOptions = { type: "full"|"fetch"|"send"; force?;
  offlineMode? }` — `type` ist **Pflicht**.
- `db.syncer.stop(): Promise<void>` — Stop (ohne Args; `db.syncer` ist public).
- `db.syncer.sync.cancel(): Promise<void>` — Cancel (über die nested `Sync`-
  Instanz, public via `db.syncer.sync`).

**Geänderte Dateien:**
- `packages/contracts/src/index.ts` — re-exportiert `SyncOptions` (war noch
  nicht drin).
- `utils/sync.ts` (neu) — pure `buildSyncOptions` (default `type:"full"`, setzt
  `force`/`offlineMode` nur wenn `!== undefined` → `exactOptionalPropertyTypes`-
  safe) + `SYNC_TYPE_LABELS` + `SyncType`/`SyncControlInput`.
- `stores/sync.ts` (neu) — `useSyncStore`: `busy`/`lastError`/`lastResult`;
  `startSync`→`db.sync(buildSyncOptions(input))` boolean, `stopSync`→
  `db.syncer.stop()`, `cancelSync`→`db.syncer.sync.cancel()`; alle never-throws.
- `commands/registry.ts` — `CommandContext`+`sync: ReturnType<typeof
  useSyncStore>`-Feld.
- `stores/command-palette.ts` — stellt `sync` im `ctx` bereit.
- `commands/app-commands.ts` — `app:sync-now`-Command (`when: showShell` →
  `ctx.sync.startSync({type:"full"})`).
- 3 Specs (`command-registry`/`router`/`toggle-commands`) — ctx-Stubs um
  `sync`-Feld ergänzt.
- `tests/contract/sync.spec.ts` (neu) — 18 Tests: pure `buildSyncOptions`/
  `SYNC_TYPE_LABELS` + Store (start default/forwarded/failure/false-result,
  stop, cancel, error-clear) + `app:sync-now` (Registrierung, Visibility,
  Run→`db.sync({type:"full"})`).

**Verifiziert (headless):** 465 Contract-Tests grün (447 + 18) über 34 Spec-
Dateien; typecheck (node+web+contracts) + `electron-vite build` clean.

**Wichtige Erkenntnisse:**
1. **Control-Store bewusst separat vom Status-Store.** Status = Event-getriebene
   *Anzeige* (`syncState`, auch Auto-Sync feuert Events ohne `startSync`-Aufruf);
   Control = `busy`/`lastResult` eines `startSync`-Aufrufs (Button-Level). Kein
   Event-Subscribe im Control-Store → isoliert testbar, kein EV-Duplikations-
   Problem (M2.6-Gotcha). View kombiniert beide.
2. **`SyncOptions.type` ist Pflicht, `force`/`offlineMode` optional.** Default
   `type:"full"`; optionale Keys nur-if-defined setzen (gleiche
   `exactOptionalPropertyTypes`-Regel wie 6.7 Backup — gilt für JEDE Store,
   der Options-Bags an Core durchreicht).
3. **`db.syncer` + `db.syncer.sync` sind public** (kein `private` im Typ) —
   `stop()` (ohne Args auf SyncManager) + `cancel()` (auf nested `Sync`) sind
   erreichbar. `db.sync()` ist der dokumentierte Start-Einstieg.
4. **`db.sync()` ist auth-gated.** Braucht Server+Token; ohne Login reject →
   `lastError`. Store funktioniert headless mit fake db; live Sync = on-site
   nach Login.
5. **`CommandContext`-Feld-Erweiterung ist viral.** Ein neues `sync`-Feld
   bedeutet: 3 Specs die ctx-Literale bauen, müssen ergänzt werden (sonst
   `TS2741` missing property). Bei künftigen Store-Commands das gleiche
   Muster erwarten.

**Aufgeschoben (on-site):** Sync-Button in StatusBar/Toolbar (folgt `busy`/
`lastResult`, löst `app:sync-now` aus), Stop/Cancel-Buttons, Error-Toast für
`lastError`.

**On-Site-Gate (physische Anwesenheit):** "Sync now" aus Palette/Button →
Status wechselt auf syncing → completed; ohne Login → Fehleranzeige.

---

## Phase 3.1 — Custom Titlebar (headless backend) (2026-07-20)

Das Custom-Titlebar-Backend. Bisher war die `BrowserWindow`-Konfiguration in
`main/index.ts` hardcoded auf macOS (`titleBarStyle: "hiddenInset"` +
`vibrancy: "under-window"` unverändert für alle Plattformen → auf Win/Linux
ungültig/ignoriert), und die `TitleBar.vue` nutzte fixes `px-3` → der Sidebar-
Toggle überlappte auf macOS die nativen Traffic-Lights. Dieses Inkrement macht
die Window-Chrome + Titlebar-Padding plattform-aware.

**Design-Entscheidung:** die Window-Controls (min/max/close) bleiben **nativ**
— der Renderer zeichnet sie NICHT selbst und IPCt sie nicht. macOS nutzt
`hiddenInset` (Traffic-Lights links inset), Win/Linux nutzen **Window Controls
Overlay** (`titleBarOverlay` — OS zeichnet Caption-Buttons rechts). Der
Renderer reserviert lediglich den Platz (`padding-left` macOS / `padding-right`
Win/Linux) so dass sein eigener Content (Sidebar-Toggle + "Notesnook"-Label)
nicht unter den nativen Buttons liegt. Genau das, was die Roadmap unter
"Window Controls Overlay (Win/Linux)" + "vibrancy/acrylic-Erkennung" fordert.

**Geänderte Dateien:**
- `contracts/titlebar.ts` (neu) — pure, dep-freie, von Main + Renderer + Tests
  geteilte Helper: `Platform`-Typ + `detectPlatform(os)` (mapt `process.platform`
  → `macos`/`windows`/`linux`/`other`), `TRAFFIC_LIGHT_INSET` (78px),
  `WINDOW_CONTROLS_FALLBACK_WIDTH` (138px), `titlebarPadding(platform,
  controlsWidth=0)` → `{left,right}` (macOS links Traffic-Light-Inset; Win/Linux
  rechts gemessene WCO-Breite sonst Fallback; `other` symmetrisch 12px base).
- `main/titlebar.ts` (neu) — `buildBrowserWindowOptions(platform, preloadPath)`
  → vollständige `BrowserWindowConstructorOptions` pro Plattform: macOS
  `hiddenInset`+`frame:false`+`vibrancy:"under-window"`+`visualEffectState`;
  Windows `hidden`+`titleBarOverlay`+`backgroundMaterial:"acrylic"`; Linux
  `hidden`+`titleBarOverlay`+`frame:false`; `other` framed. Transparenter
  `backgroundColor` + `webPreferences` (contextIsolation/nodeIntegration/sandbox
  wie bisher) zentral hier. `import type` von electron → kein Runtime-Dep.
- `main/index.ts` — ersetzt die inline `BrowserWindow`-Options durch
  `buildBrowserWindowOptionsForOS(process.platform, preloadPath)`.
- `stores/titlebar.ts` (neu) — `useTitleBarStore`: `platform` (aus `window.os`,
  guardiert für node-test), `isMacos`/`isWindows`/`isLinux`/`isDesktop`-Computeds,
  `controlsWidth`-Ref (0 → Fallback) + `setControlsWidth`, `padding`-Computed
  (delegiert an pure `titlebarPadding`), `effectiveControlsWidth`, `setPlatform`
  (Test/Mount-Hook). Bewusst **kein IPC / keine Events** — nur `window.os` →
  isoliert in node testbar.
- `components/TitleBar.vue` — bindet `padding.left`/`padding.right` via Inline-
  Style; misst auf Mount + `geometrychange` die echte WCO-Breite aus
  `navigator.windowControlsOverlay.getTitlebarArea()` (controlsWidth =
  `innerWidth − (area.x + area.width)`) → `setControlsWidth`; macOS/web → API
  absent → Fallback/kein rechter Inset.
- `env.d.ts` — ambient `WindowControlsOverlay`/`Navigator.windowControlsOverlay`
  -Typ (DOM-lib hat die API nicht).
- `tests/contract/titlebar.spec.ts` (neu) — 15 Tests: `detectPlatform` (map +
  unknown/null/`""`→other), `titlebarPadding` pro Plattform + WCO-Override +
  non-positive-Fallback + macOS-ignoriert-WCO + `other`-symmetrisch +
  non-mutating; Store (`window.os`-Detection, node-Default `other`,
  `setPlatform`→Padding-recompute, `setControlsWidth`→rechter Inset auf Win/Linux
  + 0/negative→Fallback, macOS unbeeinflusst, Computeds exklusiv).

**Verifiziert (headless):** 480 Contract-Tests grün (465 + 15) über 35 Spec-
Dateien; typecheck (node+web+contracts) + `electron-vite build` clean (main
19.58 kB / preload 1.34 kB / renderer), 0 React/theme-ui/zustand-Leck.

**Wichtige Erkenntnisse:**
1. **Kein eigener Window-Control-IPC.** WCO zeichnet die Buttons nativ; der
   Renderer reserviert nur Platz. Damit entfällt das `WindowServer`-Interface +
   Event-Forwarding das die erste Skizze vorsah — bewusst kleiner Scope, kein
   `CommandContext`-Viral, keine Preload-Event-Erweiterung. Bestehende
   `window`-Router-Stubs (`maximize`/`restore`/`minimize`/`open`/`fullscreen`/
   `list`) unangetastet.
2. **Shared pure Helper in `contracts/`.** `detectPlatform`/`titlebarPadding`
  leben in `contracts/titlebar.ts` (im node- UND web-tsconfig `include`), nicht
  in `renderer/utils` — denn Main darf `@/`-Renderer-Pfade nicht importieren
  (node-tsconfig `include` deckt `src/renderer` nicht ab) und `src/shared` ist
  renderer-seitig nicht im `include`. `contracts` ist der einzige beiden
  zugängliche Ort. Kein Duplikat, kein Node-Dep-Import in Renderer.
3. **`buildBrowserWindowOptions` ist Main-only.** Es importiert `type` von
  electron → Runtime-erased, aber der Renderer-Typgraph soll electron-frei
  bleiben (Verträge-Philosophie), also lebt es in `main/` und wird NICHT
  contract-getestet (wie sqlite/compressor/safe-storage-Server auch nicht) —
  stattdessen on-site via `npm run dev` verifiziert.
4. **`titleBarOverlay.color` transparent** (`"rgba(0,0,0,0)"`) damit das
   acrylic/vibrancy durch die Caption-Buttons-Region scheint; `height:40`
   passt zur `h-10`-Titlebar.
5. **`navigator.windowControlsOverlay` ist nur vorhanden wenn `titleBarOverlay`
   gesetzt** (Win/Linux) — auf macOS/web absent → Store behält Fallback/keinen
   rechten Inset. `getTitlebarArea()` liefert die Web-zugängliche Titlebar-Fläche
   (exkl. Controls), also controlsWidth = `innerWidth − (area.x + area.width)`.

**Aufgeschoben (on-site):** visuelle Verifikation der nativen Controls +
Glass-Blur pro Plattform (macOS Traffic-Lights frei vom Sidebar-Toggle; Win11
acrylic-Blur; Linux WCO-Buttons + frameless-Shadow/Resize), WCO-`geometrychange`
re-render bei Fenster-Resize, ggf. `trafficLightPosition`-Tuning auf macOS.

**On-Site-Gate (physische Anwesenheit):** macOS — Traffic-Lights sitzen links,
Sidebar-Toggle überlappt nicht, Vibrancy-Blur sichtbar; Windows — acrylic-Blur +
native min/max/close rechts, Titlebar-Content frei; Linux — WCO-Buttons +
frameless-Resize; Fenster lässt sich Draggen über die Titlebar, Sidebar-Toggle
klickbar (no-drag).

---

## Phase 6.5 — Deep Links `nn://` (headless backend) (2026-07-20)

`nn://`-Custom-Protocol + Routing. Bisher war `onOpenNote` vom Preload
exponiert aber nirgends konsumiert; `nn://` war gar nicht gebaut. Dieses
Inkrement registriert das Protocol beim OS, fängt URLs plattform-spezifisch ab
und öffnet die Note im Editor.

**Scheme:** `nn://<kind>/<id>` mit `<kind>` ∈ `note` | `notebook` | `monograph`
(z.B. `nn://note/abc123`). Der Parser ist generisch über alle drei Kinds
(stabil für künftige Views), aber der Main-Prozess **dispatcht aktuell nur
`note`** (Notes-View + `app:open-note`-Channel existieren); `notebook`/
`monograph` werden geloggt + gedroppt (Ziel-Views fehlen noch — Phase 3.2/6).

**Geänderte Dateien:**
- `contracts/deep-link.ts` (neu) — pure, dep-freie, von Main + Tests geteilte
  Helper: `NN_PROTOCOL`="nn", `DeepLinkKind`/`DeepLinkTarget`, `parseDeepLink(url)
  → DeepLinkTarget | null` (via `URL`-API: protocol `nn:`, hostname=kind, erste
  pathname-Segment=id; trailing-slash/query/hash toleriert; leere/multi-segment
  id → null; unbekannter kind / fremdes protocol → null; never-throws) +
  `buildDeepLink(target)`.
- `main/deep-link.ts` (neu) — `registerDeepLinkListeners` (VOR `app.whenReady`
  aufzurufen damit macOS-Cold-Start `open-url` nicht verloren geht — `preventDefault`
  + queue), `enableDeepLinkProtocol` (`app.setAsDefaultProtocolClient("nn")`,
  nach ready), `findDeepLinkInArgv` (Win/Linux-Cold-Start im argv),
  `handleDeepLinkUrl` (parse + dispatch/queue), `setDeepLinkWindow` (bindet
  Window + flusht Queue auf `did-finish-load` — `webContents.send` vor
  Page-Load wird sonst von Electron gedroppt). Dispatch: `note`→`app:open-note`
  an focussed/main window (restore+show+focus), `notebook`/`monograph`→warn.
- `main/index.ts` — `registerDeepLinkListeners()` Modul-Top (vor ready); im
  ready-Handler `enableDeepLinkProtocol()` + argv-Scan + `setDeepLinkWindow(window)`
  nach `createMainWindow`.
- `App.vue` — subscribed `window.appEvents.onOpenNote` (war exponiert aber
  ungenutzt) → `openNoteFromDeepLink(id)`: bei `auth.showShell` `router.push("/all")
  `+`notes.selectNote(id)` (Editor rendert nur auf /all), sonst in
  `pendingDeepLinkNote`-Ref queue → flush im showShell-Watch nach Login.
- `tests/contract/deep-link.spec.ts` (neu) — 10 Tests: parse note/notebook/
  monograph, trailing-slash/query/hash, leere id, multi-segment id, unbekannter
  kind, fremdes protocol, malformed-never-throws, `NN_PROTOCOL`; `buildDeepLink`
  + round-trip.

**Verifiziert (headless):** 503 Contract-Tests grün (493 + 10) über 36 Spec-
Dateien; typecheck (node+web+contracts) + `electron-vite build` clean (main
21.88 kB / preload 1.34 kB / renderer), 0 React/theme-ui/zustand-Leck.

**Wichtige Erkenntnisse:**
1. **`open-url`-Listener VOR `app.whenReady` registrieren.** macOS feuert
   `open-url` auch bei Cold-Start; ein erst nach ready registrierter Listener
   verliert die URL. `registerDeepLinkListeners()` daher Modul-Top, queue bis
   Window+Page da.
2. **`webContents.send` vor `did-finish-load` wird gedroppt.** Cold-Start
   Deep-Links dürfen nicht sofort nach `createMainWindow` gesendet werden
   (Page noch nicht geladen → kein Listener). `setDeepLinkWindow` flusht die
   Queue daher auf `did-finish-load` (oder sofort wenn `!isLoading`).
3. **Parser generisch, Dispatch selektiv.** `parseDeepLink` deckt note/notebook/
   monograph (testbar + zukunftssicher); Main sendet nur `app:open-note` für
   notes. notebook/monograph-Routing = Folge sobald deren Views stehen.
4. **Kein neuer Preload-Event.** Wiederverwendung des bestehenden
   `app:open-note`/`onOpenNote` (upstream-shaped) — kein Preload-/env.d.ts-
   Surface-Wachstum für 6.5.
5. **Editor nur auf `/all`.** Deep-Link muss vor `selectNote` nach `/all`
   routen, sonst öffnet sich ein Tab unsichtbar (z.B. während Settings-View
   aktiv).

**Aufgeschoben (on-site + Folge):** live `nn://`-Klick vom OS/Browser → App
   öffnet Note (macOS `open-url` warm/kalt, Win/Linux `second-instance` + argv);
   `notebook`/`monograph`-Routing sobald Views stehen; Deep-Link-Tests für
   Main-Dispatch (electron-Events, nicht headless testbar).

**On-Site-Gate (physische Anwesenheit):** `nn://note/<id>` aus Browser/Shell
   → App kommt nach vorn + öffnet die Note als Tab auf /all; ohne Login → Note
   öffnet nach Login/Local-Only automatisch.

---

## Phase 6.4 — System-Tray (headless backend) (2026-07-20)

System-Tray mit „New Note / New Notebook / Show / Quit". Bisher war kein Tray
gebaut. Dieses Inkrement fügt die reine Menu-Spec (testbar) + die Main-Wiring
(`Tray`+`Menu` aus der Spec) + die Renderer-Seite (Notebook-Erstellung +
Note-Erstellung via einem `app:tray-action`-IPC) hinzu.

**Geänderte Dateien:**
- `contracts/tray.ts` (neu) — pure, dep-freie, von Main + Tests geteilte
  `TrayActionId` (`new-note`|`new-notebook`|`show`|`quit`) + `TrayMenuItemSpec`
  (`{id?,label?,separator?,enabled?}`) + `buildTrayMenuSpec()` → Spec-Array in
  Roadmap-Reihenfolge (New Note, New Notebook, Separator, Show, Quit).
- `main/tray.ts` (neu) — `TrayActions`-Interface + `registerTray(window)`:
  baut `Tray` + `Menu.buildFromTemplate` aus der Spec, wiring clicks→actions;
  `show`→Window restore/show/focus, `quit`→`app.quit`, `new-note`/
  `new-notebook`→`webContents.send("app:tray-action", id)`. Tray-Icon = eingebettete
  16x16 RGBA-PNG (base64, self-contained, kein Asset-File) als `templateImage`;
  Tooltip „Notesnook"; Klick aufs Icon = Show. `destroyTray()` für Tear-Down.
  `import type` electron + `nativeImage` → Main-only, nicht contract-getestet.
- `main/index.ts` — `registerTray(window)` nach `createMainWindow`.
- `preload/index.ts` — `onTrayAction(listener)` via `app:tray-action`-Channel
  (ein Channel für beide Renderer-Aktionen statt zwei separater Events).
- `env.d.ts` — `appEvents.onTrayAction` (typed via `@contracts/tray`).
- `stores/collections.ts` — `createNotebook()` (`db.notebooks.add({title:"New
  notebook"})`→id + `load()`, never-throws → `string|null`); exportiert.
- `App.vue` — subscribed `onTrayAction` → bei `showShell`: `new-note`→
  `router.push("/all")`+`notes.create()`, `new-notebook`→`collections.createNotebook()`
  (pre-login ignoriert — Tray-„Show" bringt das Window hoch).
- `tests/contract/tray.spec.ts` (neu) — 6 Tests: Spec-Reihenfolge, einzelner
  Separator, jede Nicht-Separator-Item hat id+label, ids unique + aus dem
  `TrayActionId`-Set, Label-Wording, referentielle Stabilität.
- `tests/contract/collections.spec.ts` — +2 Tests für `createNotebook` (add+load+id
  zurück, never-throws→null bei Throw); mockDb um `notebooks.add` ergänzt.

**Verifiziert (headless):** 511 Contract-Tests grün (503 + 6 tray + 2
createNotebook) über 37 Spec-Dateien; typecheck (node+web+contracts) +
`electron-vite build` clean (main 23.63 kB / preload 1.55 kB / renderer), 0
React/theme-ui/zustand-Leck.

**Wichtige Erkenntnisse:**
1. **Ein IPC-Channel für alle Tray-Renderer-Aktionen.** `app:tray-action` mit
   `TrayActionId` statt `onNewNote`+`onNewNotebook` — ein Preload-Event,Renderer
   routed per id. Erweiterbar für künftige Tray-Einträge ohne Preload-Wachstum.
2. **`show`/`quit` in Main, `new-note`/`new-notebook` ans Renderer.** Window-
   +App-Lebenszyklus gehört dem Main-Prozess; Note/Notebook-Erzeugung dem
   Renderer (Store-Zugriff auf db + Layout/Collections).
3. **Tray-Icon als eingebettete base64-PNG.** Kein Asset-File nötig → self-
   contained; `nativeImage.createFromBuffer` + `setTemplateImage(true)` für
   macOS Light/Dark-Adaption. Platzhalter-Icon; branded Asset = on-site-Folge.
4. **`createNotebook` nie werfend.** Mirror von `notes.create()` (`db.*.add
  ({title})`+reload), return `string|null` (id bzw. null bei Throw) — Tray-Aktion
   bleibt robust auch wenn db.add scheitert.
5. **Pre-Login-Gate.** `onTrayAction`-Handler agiert nur bei `auth.showShell`;
   pre-login würde `notes.create()`/`createNotebook()` einen Tab/Notebook
   unsichtbar öffnen. Tray-„Show" bringt das Window hoch, danach Login/Local-Only.

**Aufgeschoben (on-site + Folge):** Live-Tray-Icon + Menu-Render pro Plattform
   (macOS Menu-Bar-Tray, Win/Linux Systray), branded Icon-Asset, ggf.
   „Sync now"/„New Tag"-Tray-Einträge, Tray-Reload nach Window-Neubau.

**On-Site-Gate (physische Anwesenheit):** Tray-Icon erscheint im Menu-Bar/
   Systray; Menu zeigt New Note/New Notebook/Show/Quit; New Note → öffnet
   Note auf /all; New Notebook → Sidebar listet neues Notebook; Show → bringt
   App nach vorn; Quit → App beendet.

---

## Phase 6.6 — Spell-Checker (headless backend) (2026-07-20)

Electron-`session`-Spellcheck (global enable + languages + custom dictionary).
Bisher war kein Spell-Checker gebaut. Dieses Inkrement fügt die reine
Sprach-Tabelle + Auflösungs-Logik (testbar, von Main + Renderer + Tests
geteilt), den Main-Impl (Session-API + Persistenz), den tRPC-Vertrag, den
Renderer-Store + einen Palette-Toggle-Command hinzu. Die Settings-UI ist
on-site.

**Geänderte Dateien:**
- `contracts/spell-checker.ts` (neu) — pure, dep-freie, von Main + Renderer +
  Tests geteilte `Language`-Typ + `LANGUAGES`-Tabelle (60+ Codes, verbatim vom
  Upstream `apps/desktop/src/api/spell-checker.ts`) + `LANGUAGE_REDIRECT_MAP`
  (`es`→`es-MX`, `es-419`→`es-MX`, `es-ES`→`es-AR`) + `resolveLanguage` (Code
  vs. verfügbar-Set: Redirect-Map → Bare-Tag-Fallback → `undefined`) +
  `resolveEnabledCodes` (map+filter) + `languageName`/`toLanguage`/`sortLanguages`
  (locale-aware, nicht-mutierend) + `SPELLCHECKER_ENABLED_DEFAULT=true`.
- `contracts/router.ts` — `SpellCheckerServer`-Interface
  (`isEnabled`/`languages`/`enabledLanguages`/`setLanguages`/`toggle`/
  `deleteWord`/`words`) + `registerSpellCheckerServer`/`requireSpellChecker` +
  `spellChecker`-tRPC-Router (zod-Inputs: `z.array(z.string())` für
  `setLanguages`, `z.object({enabled:z.boolean()})` für `toggle`, `z.string()`
  für `deleteWord`). Re-exportiert `Language` + die pure Helper.
- `main/spell-checker.ts` (neu) — `SpellCheckerServer`-Impl gebunden an
  `window.webContents.session` (Fallback `session.defaultSession`); persistiert
  das `enabled`-Flag nach `spellchecker.json` in userData (mirror
  `safe-storage.ts`-Pattern) + wendet es beim Register an; nutzt die pure
  `resolveEnabledCodes`/`sortLanguages`/`toLanguage` aus dem Contract. Main-only
  (Electron + node:fs) → nicht contract-getestet.
- `main/index.ts` — `registerSpellChecker(window)` nach `createMainWindow`.
- `stores/spell-checker.ts` (neu) — `useSpellCheckerStore`: `busy`/`enabled`/
  `availableLanguages`/`enabledLanguages`/`dictionaryWords`/`lastError` +
  `enabledCodes`-Computed; `refresh` (lädt isEnabled+languages+enabledLanguages+
  words parallel), `toggleSpellCheck(value)` (applied returned state),
  `setLanguages(codes)` (re-reads enabledLanguages), `deleteWord(word)`
  (entfernt lokal) — alle never-throws über `desktop.spellChecker.*`-Bridge.
  Bewusst KEIN Event-Subscribe (Session-Spellcheck feuert keine Lifecycle-Events
  → request/response wie `sync`-Control-Store, isoliert testbar).
- `commands/registry.ts` — `CommandContext.spellChecker`-Feld + Import.
- `commands/app-commands.ts` — `app:toggle-spell-check` (`when: showShell`,
  toggelt `!ctx.spellChecker.enabled`).
- `stores/command-palette.ts` — `useSpellCheckerStore` + `spellChecker` ins
  ctx-Computed.
- `App.vue` — `useSpellCheckerStore` + `refresh` im Boot + showShell-Watch.
- 5 ctx-stub-Specs (viral) aktualisiert: `router`/`sync`/`toggle-commands`/
  `command-registry`/`updater`-Spec um `spellChecker`-Feld erweitert.
- `tests/contract/spell-checker.spec.ts` (neu) — 26 Tests: pure Helper
  (`languageName`/`toLanguage`/`resolveLanguage` direkte+redirect+bare-fallback+
  undefined, `resolveEnabledCodes` filter, `sortLanguages` nicht-mutierend,
  `SPELLCHECKER_ENABLED_DEFAULT`) + Store mit `vi.hoisted`-gemockter
  `desktop-bridge` (idle/refresh-parallel/never-throws/toggle/setLanguages/
  deleteWord/enabledCodes-reactive) + `app:toggle-spell-check`-Registrierung/
  Visibility/Run-toggles-on+off.

**Verifiziert (headless):** 562 Contract-Tests grün (536 + 26) über 39 Spec-
Dateien; typecheck (node+web+contracts) + `electron-vite build` clean (main
32 KB / preload 4 KB / renderer unverändert), 0 React/theme-ui/zustand-Leck
(`electron`-Session-API nur im Main-Bundle).

**Wichtige Erkenntnisse:**
1. **`spellcheck` in Core ist per-Note, nicht global.** `db.notes.spellcheck`
   ist eine Note-Spalte (default `true`), kein `db.settings`-Accessor. Die
   globale Engine-Enable/Languages-Schicht ist daher eine reine Main-Process-
   Session-Sache (mirror Upstream `config.isSpellCheckerEnabled` + `JSONStorage`);
   der per-Note-Toggle bleibt ein Properties-Panel-Follow-up (Phase 5.1, on-site).
2. **Pure Helper MÜSSEN im Contract leben.** `LANGUAGES`/`resolveLanguage` werden
   vom Main-Impl UND (transitiv) vom Renderer-Store gebraucht; Main kann keine
   `@/`-Renderer-Pfade importieren (3.1-Gotcha). Daher `contracts/spell-checker.ts`
   dep-frei, nicht `utils/spell-checker.ts` — gleiche Begründung wie `tray`/
   `deep-link`/`titlebar`.
3. **Kein Event-Subscribe.** Im Gegensatz zu Vault/Status feuert die
   Session-Spellcheck-API keine Lifecycle-Events → kein `bindSpellCheckerEvents`,
   reines request/response wie der `sync`-Control-Store. `refresh` beim Boot +
   showShell reicht (UI pollt bei Öffnen).
4. **`toggle` returns neuen State.** Upstream `toggle` gibt `void`; wir lassen
   den Impl das angewendete `enabled` zurückgeben, damit der Store es direkt
   apply-en kann, ohne ein nachfolgendes `isEnabled`-Query (weniger
   Bridge-Roundtrips, deterministischer für Tests).
5. **`CommandContext`-Feld ist viral.** Ein neues Pflichtfeld bricht alle
   ctx-stub-Specs (5 this round) + den Palette-Store-ctx-Builder. Gleiche
   Pflicht wie `updater`/`sync`-Felder davor.

**Aufgeschoben (on-site):** Spell-Checker-UI in SettingsView (Enable-Toggle +
Sprach-Picker available↔enabled + Custom-Dictionary-Liste mit Delete-Wort),
Live-Verifikation (Tippfehler-Underline im Editor, Rechtschreib-Kontextmenü),
per-Note-Spellcheck-Toggle im Properties-Panel (Phase 5.1 Folge).

**On-Site-Gate (physische Anwesenheit):** Toggle an → Tippfehler werden im
Editor underlined; Sprachen wählen → Underline folgt der Sprache; Wort aus
Dictionary löschen → nicht mehr ignoriert.

---

## Phase 7.5 — electron-builder Packaging (config + `--dir`-Build verifiziert) (2026-07-20)

`electron-builder`-Setup für distributable Installer (macOS/Win/Linux). Bisher
war `electron-builder@^25.1.0` zwar devDep, aber ohne Config + ohne Scripts.
Dieses Inkrement fügt die Config + Scripts hinzu und verifiziert den Build
headless via `--dir` (unpacked `.app`, kein Installer). Echte Installer +
Signing + Icons bleiben on-site/release.

**Geänderte Dateien:**
- `apps/desktop/electron-builder.yml` (neu) — adaptiert vom Upstream
  `electron-builder.config.js` auf diesen Stack: `appId
  org.notesnookvue.desktop` + `productName "Notesnook Vue"` (env-overridable),
  `asar:true` + `asarUnpack: node_modules/better-sqlite3-multiple-ciphers/**`
  (einziges native Modul — M4 droppte FTS5-Extensions, `trigram` ist
  SQLite-Built-in; kein `sqlite-better-trigram`/`sqlite3-fts5-html`; Sodium =
  vendorter JS/WASM, keine `sodium-native`-Prebuilds), `files: [out/**,
  package.json]`, `protocols: [{schemes:[nn]}]` (Phase 6.5), mac
  `dmg`+`zip` arm64/x64, win `nsis`+`portable` x64, linux `AppImage`+`snap` x64,
  `publish: {provider:github, owner:marco, repo:notesnook-vue, channel:latest}`
  (treibt den Phase-6.2-Updater — electron-builder generiert `app-update.yml`
  automatisch, kein hand-gepflegtes nötig). `directories.output: release`
  (gitignored). Icons/Signing/Notarize/Cross-Arch bewusst auskommentiert +
  als on-site/release-Gates dokumentiert.
- `apps/desktop/package.json` — Scripts `package`/`package:dir`/`package:mac`/
  `package:win`/`package:linux` (alle `--publish never`); `description` +
  `author` ergänzt (electron-builder warnte); `electron` von `^37.10.3` auf
  exact `37.10.3` gepinnt (s.u.).
- **Workspace-Dep-Split** in `apps/desktop/package.json`: alle renderer-only
  Deps (`@notesnook-vue/*`, `@notesnook/*`, `@tiptap/*`, `pinia`, `vue`,
  `vue-router`, `@trpc/client`) von `dependencies` → `devDependencies`
  verschoben. In `dependencies` bleiben nur die 6 Main-Runtime-Deps die
  `externalizeDepsPlugin` externalisiert + electron-builder packt:
  `better-sqlite3-multiple-ciphers`, `electron-updater`, `electron-trpc`,
  `@trpc/server`, `zod`, `@streetwriters/kysely`.
- `package-lock.json` — zweimal via `npm install --package-lock-only`
  reconciled (electron-Pin + Dep-Split), damit `npm ci` in CI grün bleibt.

**Verifiziert (headless):** `npm run package:dir` baut ein unsigned
`Notesnook Vue.app` (mac arm64) — `@electron/rebuild` rebuilt
`better-sqlite3-multiple-ciphers` für die Electron-ABI, electron-v37.10.3-darwin
-arm64.zip geladen, `app.asar` (14 MB) enthält `out/main/index.js` +
`out/preload/index.mjs` + `out/renderer/index.html` + nur die 6 Runtime-Deps
(+ transitive: prebuild-install, tar-fs, js-yaml, …);
`better_sqlite3.node` korrekt nach `app.asar.unpacked/` entpackt;
**0 renderer-only Bloat** im asar (kein `@notesnook`/`@tiptap`/`vue`/`pinia`/
`vue-router`/`@trpc/client`/`@notesnook-vue` geleaked — der Dep-Split wirkt).
typecheck (node+web+contracts) + 562 Contract-Tests + `electron-vite build`
clean nach dem Dep-Split. Default-Icon + skipped Code-Signing (beide erwartet,
da Assets/Certs on-site).

**Wichtige Erkenntnisse:**
1. **electron-Version MUSS exact gepinnt sein.** electron-builder kann keine
   Version aus einem Caret-Range (`^37.10.3`) berechnen, wenn das
   `electron`-Modul im App-`node_modules` nicht gefunden wird (workspaces
   hoisten es nach Root). Fehler: "Cannot compute electron version … version
   (^37.10.3) is not fixed in project". Fix: exact pin + Lockfile reconcilen.
2. **electron-builder kann keine Workspace-Packages packen deren Realpfad
   außerhalb des App-Dirs liegt.** `packages/*` + die vendored
   `vendor-dist/@notesnook/*` lösen Fehler "… must be under apps/desktop/" aus
   (asar-Packer verlangt alle Dateien unterhalb des App-Dirs). Da
   Main/Preload/Contracts **keine** `@notesnook/*`/`@notesnook-vue/*` importieren
   (nur der Renderer, der sie via Vite inline-bundled), löst der Dep-Split
   (renderer-only → devDeps) das sauber: electron-builder packt nur
   `dependencies`, und `externalizeDepsPlugin` externalisiert nur `dependencies`
   → die Workspace-Packages werden weder externalisiert noch gepackt.
3. **`npm ci` bricht bei package.json-Lockfile-Drift.** Jede
   package.json-Änderung (Pin, Dep↔devDep-Verschiebung) MUSS mit
   `npm install --package-lock-only` reconciled werden, sonst schlägt
   `npm ci` in CI fehl. Braucht kein Netz für schon installierte Packages.
4. **`electron-updater` braucht `publish` in der Config.** electron-builder
   generiert `app-update.yml` aus dem `publish`-Block + backt es in die
   App-Resources ein — kein hand-gepflegtes `app-update.yml` nötig. Der
   Phase-6.2-Updater liest es zur Laufzeit.
5. **`asarUnpack` für native Module.** `better_sqlite3.node` kann nicht aus
   asar `require`'d werden → `asarUnpack: node_modules/better-sqlite3-multiple
  -ciphers/**` entpackt es nach `app.asar.unpacked/`.
6. **`--dir` verifiziert die Config end-to-end ohne Installer.** Baut das
   `.app` (bzw. Win/Linux-Verzeichnis) ohne dmg/nsis/AppImage/snap zu erstellen
   → validiert Config-Parsing + asar + native-Rebuild + Entry-Points, schnell
   + ohne Netz/Signing. Echte Installer = on-site/release.

**Aufgeschoben (on-site/release):** echte Installer (`dmg`/`nsis`/`AppImage`/
`snap` — brauchen je Target-Host + z.T. Netz für snap), branded Icons
(`build/icon.{icns,ico,png}` — fehlen, electron-builder nutzt Default-Icon +
warnt), Code-Signing/Notarize (keine Certs — `--publish never` baut unsigned),
Cross-Arch-Builds (mac x64, win/linux arm64 brauchen jew. native-Modul-Prebuilds),
Packaging-Step in CI (7.6 baut nur typecheck+test+build, nicht package — bewusst;
release pipeline = Folge), `app-update.yml`-Live-Test (Phase 6.2 on-site).

**On-Site-Gate (physische Anwesenheit):** `npm run package:mac` →
`Notesnook Vue.app` launchen → bootet end-to-end (dieselben Gates wie
`npm run dev`); `npm run package` produziert Installer pro Plattform.

---

## Phase 2.6 — i18n-Foundation (vue-i18n) (2026-07-20)

i18n-Grundgerüst für den Renderer (Entscheidung #1: **vue-i18n** statt Lingui).
Bisher waren alle UI-Strings hardcoded English (Dutzende "i18n = Phase 7.1"-
Notizen in stores/utils). Dieses Inkrement etabliert die vue-i18n-Infrastruktur
+ einen English-Katalog + eine Pseudo-Locale + migriert einen Sidebar-Slice als
Proof-of-Pattern. Die volle String-Migration ist Phase 7.1 Polish.

**Warum vue-i18n (nicht Upstream `@notesnook/intl`/Lingui):** Upstream `intl`
ist Lingui+`@lingui/macro`+React-gekoppelt — der 2810-Zeilen-`strings.ts`
importiert ein `generated/`-Codegen (aus der Upstream-React-App, hier absent),
`react` ist peerDep, und es liefert nur `en` + eine Pseudo-Locale (keine echten
Übersetzungen zu porten). Zudem bundlet `core` `intl` in sein dist → der
Renderer konsumiert `@notesnook/intl` gar nicht. Unser Vue-Renderer spiegelt die
Upstream-React-Komponentenstruktur nicht 1:1, also mappen die Upstream-Strings
ohnehin nicht sauber. Ein Vue-nativer Katalog mit eigenen Keys ist einfacher,
hat keinen Macro-Compile-Schritt + kein React-peerDep.

**Geänderte Dateien:**
- `apps/desktop/src/renderer/src/i18n/index.ts` (neu) — `createI18n({legacy:false,
  locale: readLocale(), fallbackLocale:"en", messages:{en,pseudo}})` +
  `setLocale(locale)` (switcht `i18n.global.locale.value` + persistiert nach
  `localStorage "notesnook.locale"`, best-effort try/catch wie settings-
  `themeMode`) + `LOCALES`/`Locale`/`PSEUDO_LOCALE`-Konstanten + re-exports
  `en`/`pseudo`/`toPseudo`.
- `i18n/locales/en.ts` (neu) — eigener English-Katalog (nested by feature),
  seeded mit `sidebar.notebooks`/`noNotebooks`/`tags`/`noTags` +
  `common.greeting`.
- `i18n/locales/pseudo.ts` (neu) — `toPseudo(messages)` wrappt jedes String-Leaf
  in `⟪…⟫` → Pseudo-Locale = Voll-Copy von en (App bleibt nutzbar), unveröffentlichte
  Strings sofort sichtbar. Generiert sich aus `en` → driftet nicht.
- `renderer/src/main.ts` — `app.use(i18n)` (Composition-API → `$t`/`useI18n`).
- `components/Sidebar.vue` — `useI18n()` + 4 Strings → `t("sidebar.*")`
  (Notebooks/No notebooks/Tags/No tags). Bewusst keine Komponente mit
  existierendem Mount-Test gewählt (Sidebar hat keinen → kein Test braucht das
  i18n-Plugin installiert).
- `apps/desktop/package.json` — `vue-i18n@^11.4.6` als devDep (renderer-only,
  Vite-inline-bundled → nicht von electron-builder gepackt, nicht von
  `externalizeDepsPlugin` externalisiert). Lockfile reconciled.
- `tests/contract/i18n.spec.ts` (neu) — 16 Tests (happy-dom): `toPseudo` rein,
  Locale-Konstanten, Instance (en-Katalog, pseudo wrappt, Fallback auf en für
  ungeladene Locale + fehlende Keys = total-safe), `setLocale` (switcht +
  persistiert + never-throws ohne localStorage), Initial-Read via
  `vi.resetModules`+dyn. Import (persisted pseudo→pseudo, none→en, invalid→en),
...
**Verifiziert (headless):** 578 Contract-Tests grün (562 + 16) über 40 Spec-
Dateien; typecheck (node+web+contracts) + `electron-vite build` clean (renderer
+139 KB für inline-bundletes vue-i18n, kein React/theme-ui/zustand-Leck).

**Wichtige Erkenntnisse:**
1. **`useI18n()` braucht das Plugin installiert — auch in Tests.** Komponenten
   die `useI18n()` rufen, müssen mit `app.use(i18n)` (bzw. im Test
   `mount(Comp, {global:{plugins:[i18n]}})`) gemountet werden. Daher als Slice
   eine Komponente OHNE existierenden Mount-Test gewählt (Sidebar) → kein
   bestehender Test bricht.
2. **Initial-Locale wird zur Modul-Konstruktion gelesen.** `createI18n` liest
   `localStorage` einmal beim Import. Um den Initial-Read zu testen, muss das
   Modul mit `vi.resetModules()` + dyn. Import re-evaluiert werden, NACHDEM
   `localStorage` gesetzt ist — ein simples `beforeEach` reicht nicht (Modul ist
   schon importiert/zwischengespeichert).
3. **`legacy:false` (Composition) → `i18n.global.locale` ist ein ref.** Setzen
   via `i18n.global.locale.value = "pseudo"`, nicht direkter Zuweisung. `t()`
   aus `useI18n()` in Komponenten ist reaktiv darauf → `setLocale` updatet
   gerenderte Templates nach `nextTick`.
4. **vue-i18n ist total-safe.** Fehlende Keys renderen als Key-Pfad (kein Throw);
  ungeladene Locales fallen auf `fallbackLocale` zurück. Robust für eine
   schrittweise Migration.
5. **Pseudo-Locale als Voll-Copy, nicht Teilmenge.** `toPseudo(en)` erzeugt
   jeden Key (gewrappt) → die App bleibt in pseudo voll nutzbar (kein Fallback-
   Rauschen); nur ein vergessenes `t()` (das noch hardcoded English ist) bleibt
   ungewrappt sichtbar.

**Aufgeschoben (Phase 7.1 Polish):** volle String-Migration (stores/utils/
components — `syncStatusText`, `updateStatusText`, `VAULT_ERROR_MESSAGES`,
Palette-Command-Titles, NotesList/StatusBar/LoginScreen-Texte), Locale-Switcher-
UI in SettingsView, weitere Locales (echte Übersetzungen), Architektur-Entscheid
(stores emitieren Codes + Views übersetzen = clean, vs. globale `t`-Instanz in
stores = einfacher), Pseudo-Locale über Dev-Switch aktivierbar.

**On-Site-Gate (physische Anwesenheit):** Sidebar zeigt "Notebooks"/"Tags"
(i18n-Keys); Pseudo-Locale (via `localStorage`/Dev-Switch) → Labels werden
`⟪…⟫`-gewrappt; Locale-Switch (wenn UI steht) → Texte live.

---

## Reminders-Store (headless) (2026-07-20)

`Reminder` ist eine Core-Collection (GroupingKey `reminders`) — ein
Top-Level-Item (NICHT note-gebunden): `title`, `description`, `date`, `mode`
(`once`/`repeat`/`permanent`), `priority`, und für `repeat` `recurringMode`
+ `selectedDays`. Core exponiert `db.reminders` (`add` upsert-by-id /
`all`/`active` (FilteredSelector) / `reminder(id)` / `remove(...ids)`) + reine
Helper (`isReminderActive`/`isReminderToday`/`formatReminderTime`/
`getUpcomingReminder`). Bisher gab es keinen Store, kein Util, keine Phase dafür
— dieses Inkrement landet das Headless-Backend nach dem etablierten Muster
(vault/backup/sync/note-history/trash), UI + OS-Notification-Scheduling = on-site.

**Geänderte Dateien:**
- `apps/desktop/src/renderer/src/utils/reminders.ts` (neu) — rein: Const-Arrays
  `REMINDER_PRIORITIES`/`REMINDER_MODES`/`RECURRING_MODES` + abgeleitete Literal-
  Typen (für künftige Picker-UI) + `ReminderInput`-Interface + `buildReminderInput`
  (exactOptional-safe undefined-Stripper → `Partial<Reminder>` für `db.reminders.add`;
  wendet bewusst KEINE Defaults — Core owned `mode:"once"`/`priority:"vibrate"`/
  `selectedDays:[]`) + `sortRemindersByCreatedDesc` (nicht-mutierend, stable;
  Default-Anzeige-Ordnung — echte Upcoming-Sortierung via Core-`getUpcomingReminderTime`
  ist dayjs-heavy + nicht re-exportiert → View-Sache on-site).
- `apps/desktop/src/renderer/src/stores/reminders.ts` (neu) — `useRemindersStore`:
  `items`/`loading`/`busy`/`lastError` + Computeds `count`/`activeItems` (filter via
  contracts-re-exportiertem `isReminderActive`); `refresh`=`db.reminders.all.items()`
  + `sortRemindersByCreatedDesc`; `add(input)`→`db.reminders.add(buildReminderInput)`
  → reload, return id|null; `remove(ids)`; `snooze(id,untilMs)` (upsert
  `{id,snoozeUntil}`); `toggleDisabled(id)` (read `db.reminders.reminder` → flip →
  `add`); `update(id,patch)` (generisch); alle never-throws (`lastError` bei Throw,
  `busy` in `finally`); request/response KEIN Event-Subscribe → isoliert testbar.
- `packages/contracts/src/index.ts` — Runtime-Re-export-Block um `isReminderActive`
  ergänzt (die anderen 3 Helper kommen, wenn die RemindersView sie konsumiert — nicht
  spekulativ).
- `apps/desktop/src/renderer/src/platform/bootstrap.ts` — `seedIfEmpty` seeedt eine
  "once"-Erinnerung 3 Tage aus (local-only; Accounts bleiben leer → Sync füllt), damit
  `npm run dev` die Collection + Store mit Daten treibt.
- `tests/contract/reminders.spec.ts` (neu) — 18 Tests: pure Helper (Const-Arrays,
  `buildReminderInput` stripped undefineds + keine Defaults + Edit-Patch, Sortierung
  nicht-mutierend) + Store (Start leer, `refresh` lädt+sortiert, `activeItems`
  filtert via echter `isReminderActive` (disabled/past-once excluded — `date` relativ
  zu echtem `Date.now()` da Core-Helper Wall-Clock liest), `add`+reload+id,
  `add`-null+lastError bei Throw, `remove` no-op-on-empty+reload+never-throws,
  `snooze` upsert, `toggleDisabled` flip + not-found + never-throws, `update` merge,
  `refresh` never-throws+lässt Liste intakt). Fake-`db.reminders` mit `add`-Upsert +
  Core-Defaults so dass die echte `isReminderActive` korrekt filtert.

**Verifiziert (headless):** **868 Contract-Tests grün über 62 Spec-Dateien** (+
18 reminders; 850 + 18), typecheck (node+web+contracts) + `electron-vite build`
clean. Keine neue Dep, kein Main/Preload-Change, kein App.vue-Wiring (wie Trash —
RemindersView ruft `refresh()` on-mount on-site).

**Aufgeschoben (on-site):** RemindersView (Route + Sidebar-Eintrag),
`formatReminderTime`-getriebene Listen-Anzeige, Create/Edit/Snooze/Toggle-UI,
OS-Notification-Scheduling (Main-Prozess wie Tray/Updater), echte
Upcoming-Sortierung, Picker-UIs für Mode/Priority/Recurring-Days.

---

## Colors-Store (headless) (2026-07-20)

`Color` ist eine Core-Collection (`{ title, colorCode }`) — eine Top-Level-
Sidebar-Sektion (`SideBarSection` enthält `"colors"`) + ein Note-Grouping-Key
(`GroupingByIdKey` enthält `"color"`). Core exponiert `db.colors`
(`add` upsert-by-id-oder-colorCode / `all` (FilteredSelector) / `color(id)` /
`find(code)` / `count(id)`=Notiz-Count via `relations.from(color,"note").count()`
/ `remove` (soft-delete + `unlinkOfType("color",…)` in einer Transaktion)).
`DefaultColors` (7er-Palette) + `Color`-Typ sind bereits aus `@notesnook/core`
in Contracts re-exportiert. Bisher gab es keinen Store, kein Util — dieses
Inkrement landet das Headless-Backend nach dem etablierten Muster
(reminders/tags/properties). Note↔Color-Zuweisung läuft via `db.relations`
(`Note.color` ist `@deprecated`/migrations-only) → Zuweisung bleibt ein
Properties-Panel-Folge (wie Tags/Notebooks dort); dieser Store managed die
Collection selbst.

**Geänderte Dateien:**
- `apps/desktop/src/renderer/src/utils/colors.ts` (neu) — rein: `ColorInput` +
  `buildColorInput` (exactOptional-undefined-Stripper → `Partial<Color>` für
  `db.colors.add`; keine Defaults — Core owned sie + throwt bei fehlendem
  title/colorCode) + `ColorListItem` + `toColorListItem` (Untitled-Fallback) +
  `sortColorsByTitle` (case-insensitive, nicht-mutierend, stable).
- `apps/desktop/src/renderer/src/stores/colors.ts` (neu) — `useColorsStore`:
  `items`/`loading`/`busy`/`lastError` + `count`-Computed; `refresh`=
  `db.colors.all.items()`+`sortColorsByTitle`; `add(input)`→`db.colors.add(
  buildColorInput)`→reload→id|null; `remove(ids)` (no-op-on-empty);
  `noteCount(id)`=`db.colors.count(id)`→number (0 bei Miss/Throw); alle
  never-throws (`lastError` bei Throw, `busy` in `finally`); request/response
  KEIN Event-Subscribe → isoliert testbar.
- `packages/contracts/src/index.ts` — **keine Änderung** (`Color` +
  `DefaultColors` bereits re-exportiert).
- `apps/desktop/src/renderer/src/platform/bootstrap.ts` — `seedIfEmpty` seeedt
  eine Custom-Color "Indigo" `#5C6BC0` (local-only; Accounts bleiben leer → Sync
  füllt), damit `npm run dev` die Collection + Store mit Daten treibt.
- `tests/contract/colors.spec.ts` (neu) — 15 Tests: pure Helper
  (`buildColorInput` stripped undefineds + keine Defaults + id-Patch,
  `toColorListItem` Untitled-Fallback, `sortColorsByTitle` case-insensitive
  nicht-mutierend) + Store (Start leer, `refresh` lädt+sortiert, `add`+reload+id,
  `add`-null+lastError bei Throw, `add` upsertet existierende Color by colorCode
  ohne Duplikat, `remove` no-op-on-empty+reload+never-throws, `noteCount` seeded
  count + 0-on-miss + never-throws, `refresh` never-throws+lässt Liste intakt).

**Verifiziert (headless):** **883 Contract-Tests grün über 63 Spec-Dateien**
(+ 15 colors; 868 + 15), typecheck (node+web+contracts) + `electron-vite build`
clean. Keine neue Dep, kein Main/Preload-Change, kein Contracts-Change, kein
App.vue-Wiring (wie Trash/Reminders — Sidebar "colors"-Sektion ruft `refresh()`
on-mount on-site).

**Aufgeschoben (on-site):** Sidebar "colors"-Sektion (Sektion-Order via
`sideBarOrder:colors` + `sideBarHiddenItems`), Note-Color-Picker-UI,
Note↔Color-Zuweisung (Properties-Panel `addColor`/`removeColor` via
`db.relations.add({color},{note})` color→note-Richtung / `db.relations.unlink`,
Read via `db.relations.to(note,"color").resolve()`), Note-List-Grouping nach
Color, `DefaultColors`-Palette vs. Custom-Colors-Render, per-Color-Note-Count-
Anzeige (`noteCount`).

---

## Shortcuts-Store + Sidebar-Pin-Funktionalität (headless store + UI, on-site visual gate) (2026-07-20)

`Shortcut` ist eine Core-Collection die ein Notebook/Topic/Tag an die Sidebar
pinn (`Shortcut = { itemId, itemType: "tag"|"notebook", sortIndex }`,
shortcut-id = itemId). Core exponiert `db.shortcuts` (`add` upsert-by-itemId,
throwt ohne itemId/itemType + bei nicht-erlaubtem itemType / `all` SYNCHRON
cached → `Shortcut[]` / `shortcut(id)` / `exists(id)` / `remove` / `resolved
(type="all"|"notebooks"|"tags")` → `Notebook[]|Tag[]` in dateCreated-Reihenfolge).
Upstream `ALLOWED_SHORTCUT_TYPES = ["notebook","topic","tag"]` — **Notizen sind
upstream NICHT shortcut-fähig** (und wir divergieren nicht vom Upstream-Schema,
da wir periodically pullen). `sortIndex` ist upstream tot (immer `-1`,
`resolved()` sortiert nach `dateCreated`) → kein Reorder implementiert.
Bisher: kein Store, kein Util, keine Sidebar-Sektion.

**Geänderte Dateien:**
- `apps/desktop/src/renderer/src/utils/shortcuts.ts` (neu) — rein:
  `SHORTCUT_ITEM_TYPES = ["notebook","tag"] as const` (der vendored-TS-Subset;
  `topic` runtime-erlaubt aber nicht im flachen Sidebar gerendert) +
  `ShortcutItemType` + `ShortcutInput` + `buildShortcutInput`
  (exactOptional-undefined-Stripper, keine Defaults — Core throwt ohne
  itemId/itemType) + `ResolvedShortcut` (`{id,title,type}`) +
  `toResolvedShortcut(item)` (Discriminator via BaseItem `type`) +
  `sortShortcutsByCreated` (dateCreated-asc, nicht-mutierend).
- `apps/desktop/src/renderer/src/stores/shortcuts.ts` (neu) —
  `useShortcutsStore`: `items`/`resolved`/`loading`/`busy`/`lastError` +
  `shortcutIds`-Computed (Set der itemIds, O(1)-Membership) ; `refresh`
  (sync `db.shortcuts.all` + async `db.shortcuts.resolved()`→map, never-throws);
  `add(itemId,type)`→`db.shortcuts.add(buildShortcutInput)`→reload→id(=itemId)
  |null; `remove(itemId)`; `toggle(itemId,type)` (pin-if-absent else unpin);
  `isShortcut(id)` sync; alle never-throws, request/response kein Event-Subscribe.
- `apps/desktop/src/renderer/src/components/Sidebar.vue` — neue
  **Shortcuts-Sektion** (nur wenn `shortcuts.resolved.length>0`, nach den
  top-links, expandierbar via lokalem `shortcutsCollapsed`-ref; Rows: 📓/`#` +
  Titel, Klick→`selectCollection`, hover-revealed ✕-Unpin-Button) +
  **★/☆ Pin-Toggle** auf jeder Notebook-/Tag-Row (hover-revealed `<span
  @click.stop>` → `toggleShortcut`, `group`-class für hover, active-State via
  `shortcuts.isShortcut(id)`, `★` amber wenn gepinnt; neben dem bestehenden 📌
  pinned-sort-Indicator der ein anderes Konzept ist). `toggleShortcut(type,id)`
  Helper (CollectionType == ShortcutItemType).
- `apps/desktop/src/renderer/src/i18n/locales/en.ts` — `sidebar.shortcuts`/
  `noShortcuts`/`addToShortcuts`/`removeFromShortcuts` (pseudo auto-wraps).
- `apps/desktop/src/renderer/src/App.vue` — `shortcuts.refresh()` neben
  `collections.load()` im Boot + showShell-Watch + syncCompleted-Watch.
- `packages/contracts/src/index.ts` — **keine Änderung** (`Shortcut`+
  `Notebook`+`Tag` bereits re-exportiert).
- `apps/desktop/src/renderer/src/platform/bootstrap.ts` — `seedIfEmpty` pinned
  das geseeedte Notebook + Tag als Shortcuts (local-only) → Shortcuts-Sektion +
  ★-Toggles haben Daten auf `npm run dev`.
- `tests/contract/shortcuts.spec.ts` (neu) — 14 Tests: pure Helper
  (`SHORTCUT_ITEM_TYPES`, `buildShortcutInput` stripped+keine-Defaults,
  `toResolvedShortcut` notebook/tag/Untitled, `sortShortcutsByCreated`
  nicht-mutierend) + Store (Start leer, `refresh` raw+resolved in dateCreated-
  Reihenfolge, `add`+reload+id=itemId, `add`-null+lastError bei Throw, `remove`
  by-item-id+reload+never-throws, `toggle` pin-then-unpin, `isShortcut` sync,
  `refresh` never-throws+lässt Zustand intakt). Fake-`db.shortcuts` modelliert
  sync `all`-Getter + upsert-by-itemId + `resolved()` via
  `db.notebooks/tags.all.items(ids)`.

**Verifiziert (headless + UI wiring):** **897 Contract-Tests grün über 64 Spec-
Dateien** (+ 14 shortcuts; 883 + 14), typecheck (node+web+contracts) +
`electron-vite build` clean. Keine neue Dep, kein Main/Preload/Contracts-Change.

**Aufgeschoben (on-site visual gate):** Shortcuts-Sektion rendert die gepinnten
Notebooks/Tags + Click→Filter-Route + ✕-Unpin; ★/☆-Pin-Toggle auf Notebook/Tag-
Rows toggelt + Sektion aktualisiert live; Klick-Auswahl-Highlight einer
Shortcut-Row. (Topics als Shortcut-Typ = Folge, sobald Sub-Notebooks gerendert
werden.)

---

## Sub-Notebooks (nested notebooks via `db.relations`, lazy tree + UI, on-site visual gate) (2026-07-20)

Die Sidebar renderte Notebooks bisher **flach** (Subnotebooks aufgeschoben laut
`utils/collections.ts`-Header + Memory `phase3-2-sidebar-collections`). Upstream
modelliert Sub-Notebooks als **verschachtelte Notebooks via `db.relations`** —
NICHT als `Topic` (`Notebook.topics` ist `@deprecated`, die `Notebooks`-Klasse hat
keine `topics()`-Methode). Die Relation ist **parent→child**:
`db.relations.add({type:"notebook", id: parent}, {type:"notebook", id: child})`
(bestätigt via `db.notebooks.roots` = Notebooks die nie ein `toId` sind = Top-Level,
`breadcrumbs(id)` wandert `fromId` aufwärts = Parents, und der `withSubNotebooks`-
CTE). Children eines Notebooks = `db.relations.from({type:"notebook", id},
"notebook").resolve()`. **`db.notebooks.notes(id)` rekursiert bereits alle
Descendants** (via `withSubNotebooks`-CTE) → das Auswählen eines beliebigen
Notebooks filtert seine ganze Sub-Tree-Noten — **keine Notes-Store-Änderung
nötig** (`notes.filterByCollection` ruft schon `db.notebooks.notes(id)`).

**Geänderte Dateien:**
- `apps/desktop/src/renderer/src/utils/collections.ts` — neue reine Tree-Helper:
  `NotebookTreeNode` (`{item, children}`) + `buildNotebookTree(roots, childrenOf-
  Map, key, dir)` (rekursiv nesten, Children sortiert via bestehendem
  `sortCollections` wiederverwendet — nicht re-implementiert; Blätter `children:[]`;
  nicht-mutierend). Header-Kommentar aktualisiert (Sub-Notebooks nicht mehr
  "deferred"). Bestehende Mapper/Sort unverändert.
- `apps/desktop/src/renderer/src/stores/collections.ts` — `notebooks` bleibt
  **all** (flat, für `selectedLabel`-Lookup/Counts/Empty-State); neu `roots`
  (`db.notebooks.roots.items()`), `children: Record<id, NotebookListItem[]>`
  (lazy), `expanded: Set<id>`; `notebookCount`-Computed (all) + `treeNotebooks`-
  Computed (`buildNotebookTree(roots, children, key, dir)`); `load()` lädt
  all+roots+tags+trash parallel; `loadChildren(id)`=
  `db.relations.from({type:"notebook",id},"notebook").resolve()`→map+sort→
  `children[id]` (never-throws); `toggleExpand(id)` (lazy load-then-flip,
  idempotent); `createSubNotebook(parentId)`=`db.notebooks.add`+
  `db.relations.add({parent},{child})`+`loadChildren`+auto-expand+all-reload
  (never-throws→id|null); `createNotebook` bleibt (Root).
- `apps/desktop/src/renderer/src/components/NotebookNode.vue` (neu, rekursiv) —
  `defineOptions({name:"NotebookNode"})` für Selbst-Referenz; Props `{node,depth}`;
  nutzt `useCollectionsStore`+`useShortcutsStore`+`useNotesStore`+`useRouter`
  direkt (kein Event-Plumbing); Row: Chevron (rotiert, nur wenn `children.length>0`,
  `@click.stop`→`toggleExpand`, sonst Spacer) + 📓 + Titel (Klick→`select`=
  `collections.select`+`notes.filterByCollection`+`router.push('/all')`) + 📌
  wenn `pinned` (bestehendes pinned-sort-Konzept, ≠ ★-Shortcut) + hover-revealed
  ＋ (`createSubNotebook`) + ★/☆ (`shortcuts.toggle`); `paddingLeft`=`depth*12+8`px;
  rekursiert `<NotebookNode>` über `node.children` wenn `expanded.has(id)`.
- `apps/desktop/src/renderer/src/components/Sidebar.vue` — flat Notebook-`v-for`
  ersetzt durch `<NotebookNode v-for="node in collections.treeNotebooks" :depth=0>`;
  Header-Count→`notebookCount` (all); Empty-State→`notebookCount===0`; inline
  Notebook-Row + dessen ★-Toggle entfernt (beides in `NotebookNode`); `NotebookNode`
  importiert. Tag-Rows + Shortcuts-Sektion unverändert.
- `apps/desktop/src/renderer/src/i18n/locales/en.ts` — `sidebar.createSubNotebook`
  (pseudo auto-wrapt).
- `apps/desktop/src/renderer/src/platform/bootstrap.ts` — `seedIfEmpty` seeedt
  ein Sub-Notebook "Sub-topic" unter "Getting started" (child notebook +
  parent→child-Relation + eine Welcome-Note in den Child, local-only) →
  `npm run dev` zeigt den Tree + Descendant-Filtering.
- `tests/contract/collections.spec.ts` — Fake-`db` erweitert: `notebooks.roots`
  + `db.relations.from(ref,"notebook").resolve()` (backed by `_childMap`) +
  `db.relations.add` (verlinkt child, entfernt es aus roots); 7 neue Tests:
  `buildNotebookTree` nestet+sortiert Children+Blätter, `load` populates roots+all,
  `loadChildren` via relations, `loadChildren` never-throws+lässt Previous,
  `toggleExpand` lazy-load-then-flip+idempotent, `createSubNotebook` add+relation+
  reload+expand, `createSubNotebook` never-throws. (Bestehenden `createNotebook`-
  Test an realistischeres Fake-`add` (pusht in `_all`) angepasst.)

**Verifiziert (headless + UI wiring):** **904 Contract-Tests grün über 64 Spec-
Dateien** (+ 7 in `collections.spec.ts`; 897 + 7), typecheck (node+web+contracts) +
`electron-vite build` clean. Keine neue Dep, kein Main/Preload/Contracts-Change,
keine Notes-Store-Änderung (`db.notebooks.notes` rekursiert Descendants).

**Aufgeschoben (on-site visual gate):** Tree-Expand/Collapse-Render + Indentation;
＋ Create-Sub-Notebook fügt ein Child hinzu + expandiert; Parent-Auswahl filtert
Parent+Child-Noten, Child-Auswahl nur Child-Noten (Descendant-Rekursion);
★-Shortcut-Toggle funktioniert auf Tree-Rows; Re-Parenting + Delete-Sub-Tree =
Folge (nicht in diesem Inkrement). Tags bleiben flach (kein Upstream-Hierarchie).
Topics als Shortcut-Typ = Folge, sobald Sub-Notebooks gerendert werden (jetzt
gegeben → Shortcuts könnten Topics zulassen, Folge-Inkrement).
...
## Global Search (Title-Bar, Lexical-Phase) + Scroll-to-Match + Reuse-Existing-Tab (2026-07-22)

Komplett neue ** globale Suche im Fenster-Titelbar** (Search zieht aus der Notes-Liste raus). Lexikal-Phase reuses den vendored Core-FTS5/BM25-Backend (`db.lookup.notesWithHighlighting` — im Core vorhanden, vom Renderer bisher **nie** angerufen); Semantic/Embedding-Phase später (Store ist der Swap-Punkt). Backend: `db.lookup.notesWithHighlighting(q, db.notes.all, {sortBy:"relevance", sortDirection:"desc"})` → `VirtualizedGrouping<HighlightedResult>` (`.length` + `async .item(i)→{item}`); `HighlightedResult.title:Match[]`/`content:Match[][]`, `Match={prefix,match,suffix}` (Match-Text pre-isoliert). FTS5 über Titel+Body, BM25-gerankt, DB-Trigger-gehalten-sync; vault-locked Notes haben leere `content_fts` (nur Title-Matches möglich).

**UX:** Tippen im Titelbar-Input → debounced (180ms) `runSearch` → geranktes Dropdown (Note-Titel + gehighlighteter Body-Snippet, keyboard-navigable Arrows/Enter/Esc). Pick eines Results → Note im Tab geöffnet + **zum Match gescrollt**. Enter ohne Auswahl (oder "View all results") → **"Search Results"-Tab** (`kind:"search"`) mit strukturierter Fundstellen-Liste (jede einzeln in eigenen Tab öffnbar). Hotkey **Ctrl/Cmd+Alt+F** (plain Cmd/Ctrl+F = per-pane find-in-note; Shift+P = Palette — Alt+F unclaimed + liest als "find"). Subtiles platform-aware Kbd-Hint im Input (`⌘⌥F`/`Ctrl+Alt+F`), versteckt wenn fokussiert/getippt.

**Neue Dateien:** `stores/search.ts` (headless `useSearchStore` — query/open/loading/results/activeIndex/resultsCache LRU8/focusSignal, `runSearch`/`fetchResults`/`loadResults`, `openNoteAt`/`openResult`/`openResultsTab`/`next`/`prev`/`focus`/`close`/`clear`). `components/GlobalSearchInput.vue` (Titelbar-Input + Kbd-Hint + Keyboard-Handler + `SearchDropdown`-Anchor). `components/SearchDropdown.vue` (teleported, CommandPalette-Pattern, rows via `matchesToHtml`/`snippetHtml` als `v-html` — safe, Fragmente escaped). `components/SearchResultsPane.vue` (`kind:"search"`-Tab-Inhalt, liest `resultsCache[q]`/`loadResults`). `utils/search-scroll.ts` (`scrollEditorToMatch`). `contracts/search.ts` (`@contracts/search` — pure `escapeHtml`/`matchesToHtml`/`snippetHtml`; re-export `HighlightedResult`/`Match`).

**Geänderte Dateien:** `editor-layout.ts` — `EditorTab.kind:"note"|"attachment"|"search"` + `searchQuery?` + `openSearchTab(q)` (dedup-by-query via `tabForSearch`). `openTabForce` **entfernt** (Search reused statt force-new — siehe Reuse unten). `editor.ts` — `pendingScrollTargets` shallowRef (keyed by **tabId**, nicht noteId) + set/get/clear + `getEditor(key)`. `Editor.vue` — konsumiert pending Target AM ENDE von `loadCurrentNote` direkt nach `setContent`-Chain + in `onActivated` (Reuse-Fall); `EditorPane.vue` 3-way KeepAlive-Dispatch (note+attachment cached; SearchResultsPane außerhalb KeepAlive). `NoteTabs.vue`+`notes.ts` (lokales `EditorTab`-View-Model) — Search-Tab-Titel `"Search: "+searchQuery`. `TitleBar.vue` — `<GlobalSearchInput>` statt "Notesnook"-Label + Ctrl/Cmd+Alt+F-Listener. `NotesList.vue` — Search-Row entfernt (input/regex/clear), New-Note-`+` in Count/Sort-Row re-homed, `focusSearchSignal`-Watch + `searchInput`-ref + `watch`-import gedroppt. `app-commands.ts` `app:search-notes` → `ctx.search.focus()`; `registry.ts` `CommandContext.search`; `command-palette.ts` wired `useSearchStore()`. `contracts/session-state.ts` — persisted `EditorTab`+`EditorTabSchema` (zod `z.enum(["note","attachment","search"])` + `searchQuery`) → Search-Tabs persistieren (SearchResultsPane re-fetcht via `loadResults`).

**Layering-Gotcha:** renderer-lokal Contracts = `apps/desktop/src/contracts/*` (Alias `@contracts/*` in tsconfig.web + vitest.config). `packages/contracts/src/*` ist das `@notesnook-vue/contracts`-Workspace-Pkg. `search.ts` lebt unter `apps/desktop/src/contracts/search.ts`.

**Snippets sind TEXT nicht HTML** (via htmlparser2 `decodeEntities:true` extrahiert) — render NUR durch `matchesToHtml` (escaped + `<mark class="find-match">`-Wrap), NIE `v-html` auf rohe Fragmente. `find-match`-CSS reused vom find-replace. `matchIndex` ist APPROXIMATION (FTS liefert Snippet-Blocks = Fenster um Matches, nicht jedes Vorkommen → results-tab snippet-index ≈ Nth occurrence, geclampt). Title-only-Matches scrollen nicht (Title ist separates Feld, nicht im Editor-Body-Doc); nur Content-Matches scrollen. `findMatches` ist literal-substring (`buildRegex` escaped im non-regex-Mode) → trigram-matchte Substrings resolven.

**Hotkey-Gotcha (macOS):** check `e.code === "KeyF"`, NIE `e.key === "f"` — Option/Alt ist Dead-Modifier auf macOS, ändert `e.key` zu Glyph (Alt+F → "ƒ") → `e.key`-Check feuert nie.

**Scroll-to-Match — DREI unabhängige Bugs, jeder musste einzeln gefixt werden** (lange Debug-Saga):
1. **Konsum NACH `setContent`, nicht auf Content-Flag-Watcher.** `myContentState` (`notes.getContent(id).state`) kippt auf `"loaded"` wenn `notes.loadContent(id)` resolved — passiert INSIDE `loadCurrentNote`s `await Promise.all(...)`, VOR dem `inst.chain().setContent(html).run()` das den Editor-Doc füllt. Ein entkoppelter `scrollReady`-Watcher (gated auf `myContentState==="loaded"`) konsumierte das Target gegen einen LEEREN Doc (`docSize=2`, 0 Matches). Fix: Konsum INSIDE `loadCurrentNote` direkt nach `setContent`-Chain (Doc garantiert geseedet). Der `scrollReady`-Watcher wurde entfernt.
2. **ProseMirror `tr.scrollIntoView()` scrollt den FALSCHEN Node.** Scrollt `view.scrollDOM` (`.ProseMirror`), aber hier hat `.ProseMirror` keinen Overflow — der echte Scroller ist der Ancestor `<div class="overflow-y-auto p-6">` in `Editor.vue`. Also scrollte `scrollIntoView` einen non-scrollable Node → Viewport bewegte sich nie (Matches gefunden + scroll "dispatched" aber nichts passierte). Fix: `scrollEditorToMatch` scrollt MANUELL — `findScrollContainer` walkt von `view.dom` hoch zum ersten Ancestor mit `overflow-y: auto/scroll` + bounded height, `view.coordsAtPos(from)` liefert Viewport-Coords → in Scroller-Offset konvertiert → `scrollTop` setzt um Match zu **zentrieren**. (jsdom-Tests zeigen `scrolled=false` — keine echte Layout — aber Selection-Dispatch-Asserts passen.)
3. **Async-Image-Layout-Shift.** Image-Node-Views rendern zur `setContent`-Zeit als 0×0-Placeholder (Blobs downloaden + `<img load>` feuert async) → ein Match am Doc-Ende mappt erst NACH Image-Load auf eine hohe DOM-Stelle. Fix: `scrollEditorToMatch` re-scrollt über ein Fenster — raf + Timeouts `[60,180,380,700,1200]ms`, jeder Retry guarded durch `stillOnMatch()` (checkt `editor.state.selection.from === target` → User click/type stoppt das Yanking). Doc-Position `from`/`to` ist STABIL über Image-Loads (ProseMirror-Model-Positionen; Images sind Nodes an fixen Positionen) — nur die DOM-Mapping ändert sich, die der Re-Scroll korrigiert.

**Reuse-Existing-Tab (User-Request nach force-new):** `openNoteAt` nutzt `layout.openTab` (reuse-or-create) statt dem entfernten `openTabForce`. Pick eines Results für eine **bereits offene** Note → aktiviert diesen Tab + scrollt zum Match (kein Duplicate-Tab). Drei Konsum-Paths, alle keyed by tabId: (1) Note aktuell sichtbar (editor live + `view.dom.isConnected`) → `openNoteAt` scrollt direkt (kein Lifecycle-Hook feuert); (2) Tab war KeepAlive-cached/deaktiviert (`isConnected` false) → Target bleibt staged, `Editor.vue` `onActivated` konsumiert nach `await reloadIfStale` (stale-refresh kann Doc nicht unter dem Scroll resetten); (3) Brand-neuer Tab (editor nicht registriert) → Target bleibt staged, `loadCurrentNote` konsumiert nach `setContent`. Enabler: `onDeactivated` unregistriert den Editor NICHT (nur `onBeforeUnmount`) → cached Tabs Editor bleibt in `editorStore.registry` findable via `getEditor(tabId)`; `view.dom.isConnected` unterscheidet live-vs-cached.
