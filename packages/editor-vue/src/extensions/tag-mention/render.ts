/**
 * Suggestion `render` contract for the `#` tag picker. Mounts `TagMenu.vue`
 * via TipTap's `VueRenderer`, updates its props as the query changes, forwards
 ...
 * Suggestion plugin fires `onExit`). `onExit` destroys the renderer. Twin of
 * `slash-commands/render.ts`.
 */
...
type RenderProps = SuggestionProps<TagSuggestionItem, TagSuggestionItem>;
...
export function tagMenuRenderer(): {
  ...
  return {
    ...
    onKeyDown({ event }: SuggestionKeyDownProps): boolean {
      ...
      if (event.key === "Escape") {
        // Delete the `#`-trigger + query so the suggestion decoration is
        // removed → the plugin fires onExit → popup unmounts.
        const p = state.props;
        if (p) {
          p.editor.chain().focus().deleteRange(p.range).run();
        }
