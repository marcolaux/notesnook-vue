/*
Audio node — an atom block node for inline-playable audio attachments, modelled
on extensions/image/image.ts (GPL-3.0 port lineage). The ProseMirror schema
(addAttributes/parseHTML/renderHTML) mirrors the image node so an
`<audio src width height align data-align data-hash data-filename data-mime
...
`<audio controls>` player (AudioComponent) that lazy-loads the encrypted blob
...
The Standard Notes importer emits hash-only `<audio data-hash …>` nodes for
audio `snfile`/`inline-file` attachments; the player resolves the blob on
view. Notesnook has no prior native audio node — this fills that gap so SN
audio imports play inline with the app's design pattern.
*/
import {
  Node,
  mergeAttributes,
...
declare module "@tiptap/core" {
  interface Commands<ReturnType> {
    audio: {
      ...
      setAudioAlignment: (options: AudioAlignmentOptions) => ReturnType;
      setAudioSize: (size: AudioSize) => ReturnType;
    };
  }
}
...
export const AudioNode = Node.create<AudioOptions>({
  ...
  parseHTML() {
    return [
      // A `<p>` wrapping an `<audio>` is skipped so the audio parses as a block,
      // not as paragraph content (inline audio is not supported — `inline`
      // defaults to false). Mirrors the image node's inline→block migration.
      {
        ...
        skip: true,
        getAttrs(node) {
          if ((node as HTMLElement).querySelectorAll("audio").length <= 0)
            return false;
          return null;
        }
      },
      {
        tag: this.options.allowBase64 ? "audio" : 'audio:not([src^="data:"])'
      }
    ];
  },

  renderHTML({ HTMLAttributes }) {
    // `<audio>` is a void element — a single-element array renders no closing
        ...
        ({ commands, state }) => {
          ...
          if (selectedNode && selectedNode.type.name === this.name) {
            return commands.insertContentAt(
              ...
              audios.map((a) => ({ type: this.name, attrs: a }))
            );
          }
