...
- `npm run test:contract` — **1847/1847 tests pass** across 123 files (new `editor-context-menu.spec.ts`, `editor-block-link.spec.ts`, `deep-link-paste.spec.ts`, `notes-sync-incremental.spec.ts`; extended `daily-notes.spec.ts`).
- On-site gates pending: editor context menu (clipboard copy/cut/paste + clipboard permissions, formatting toggles, link create/edit/remove via the dialog, replace-in-note opening the replace row); copy deep-link-to-block + paste-back as a titled link; `Cmd/Ctrl+D` opening today's daily note; cross-device sync applying incrementally (no chip/tint flicker, new/deleted/trashed/archived notes handled, >32 fallback); daily-notes timeline counter + panel agreement across the three channels; suggestion popups closing on tab switch; horizontal wheel scroll on the toolbar/tab strip.
