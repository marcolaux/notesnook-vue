...
### ✨ Attachment manager + image-compression preference are now per-account
The Attachments settings section — browse attachments by media type, see which notes use each, find and delete orphaned ones — used to live under the device-wide **Global** settings group, even though attachment data is already stored per-account (each account opens its own encrypted SQLite, so `db.attachments` always reads the active account's). It now sits under **each account group** in the Settings sidebar (Local + every logged-in account), between Search and Vault, so the attachments you browse are always the selected account's — clicking it under a different account live-swaps the DB singleton (the same context-switch the other per-account sections use). Because the same section component stays mounted when you move between two accounts' Attachments, it now watches the context-change signal and reloads from the new account's DB (instead of only loading once on mount), resetting the per-row expand state and clearing cached linked-note usage so a content-addressed hash collision can't leak one account's notes into another. The Global group is now Sync / Backup / Import / Updates.

The section also gains a real per-account preference: **Image compression** (Ask every time / Compress / Don't compress), previously a dormant device-global config key with no UI. It now lives in the Attachments section and is stored per-account (ctx-namespaced localStorage, with a lazy legacy-fallback migration from the old un-suffixed key), so each account can decide independently whether pasted/dropped images are compressed; the value reloads on an account switch via `config.loadClientPrefs`. `settings.attachments.imageCompression*` added (en/de); `AttachmentsSection` gained a `useAuthStore` context-signal watch + a `useConfigStore` select; `clearUsage` added to the attachments store. 4 new contract tests (64 in the touched files pass, 1998 overall — 1 pre-existing flaky canvas-theme MutationObserver timing test); typecheck clean.

## [0.20.0] - 2026-08-02

### 🛠 Fix the account-wide vector visualizer after the multilingual model swap
The per-note semantic map landed one day after the embedding model swap (all-MiniLM-L6-v2 → granite-embedding-97m-multilingual, CLS pooling, int8), and the account-wide visualizer broke in its wake: clusters vanished and a thicket of lines connected nearly every note. The two views are independent (they share only stateless primitives), but the visualizer's hard-coded defaults were tuned for the old model — granite's tighter cosine distribution pushed far more pairs over the old 0.6 edge cutoff (an O(n²) all-pairs pass) and made DBSCAN's eps=0.35 either merge everything or label it all noise. **Edges are now kNN** (each node links to its top-4 similar, gated by a `similarityThreshold` that's now a *floor* rather than an all-pairs cutoff, ≈ K·N/2 edges instead of N²), the defaults are recalibrated for granite (`dbscanEps` 0.35 → 0.28, threshold 0.6 → 0.65), and the 2D PCA projection is **seeded** so the layout stops jumping on every rebuild (it had used `Math.random()` for its initial PC guess). A new `isReindexing` flag tracks the model-change reindex **per note** (a pending-id set seeded as each note is re-queued, cleared as its embedding actually lands — on success *or* hard failure, so a stubborn note can't hold it open forever) so the "Reindexing semantic search" banner in the visualizer stays up until the batch is truly drained — not just until the queue briefly empties (which a coincidental edit could cause mid-reindex) — and the graph auto-refreshes once it clears. The reindex is now **resumable**: a `pending` marker is written before the drain starts and the model key is stamped only after it completes, so an app closed mid-reindex resumes via a catch-up on the next boot (no re-purge) instead of silently leaving notes un-indexed or re-doing everything from scratch. The same banner now also covers the per-note visualizer in the right sidebar and the omnibar (cluster tier), not just the full-graph modal. An account/context switch now **aborts** any in-flight reindex and drops the pending embedding queue, and the idle drain re-checks a generation counter each iteration — so the old account's queued embeddings (a long reindex drain in particular) can't write into the new account's `vec_notes` (`runSql` otherwise retargets to the new context's db on every call). 16 new contract tests cover the reindex state-machine decision and the kNN-edge / PCA-determinism / clustering maths (1997 pass); typecheck clean.

### ✨ Resize images in the editor + align them from a floating row
Inline images already had a bottom-right corner drag handle to resize, but its icon was a broken garbled glyph and there was no way to align or quickly size an image without dragging. The handle now shows a clean Lucide resize glyph, and selecting an image opens a floating one-row toolbar above it with **alignment** (left/center/right) and **size presets** (S / M / L / Full / Original). Presets snap to 25/50/75/100% of the editor width (Original returns to the image's natural pixels), while the corner drag still fine-tunes — combining upstream's split of a corner drag and a separate numeric properties popup into one tidy row. The toolbar commits through the existing `setImageSize`/`setImageAlignment` commands (undoable, and they re-select the node so the row stays open across clicks); it's hidden in read-only mode. `ImageSize` widened to `number | null` to match the reset idiom; natural dimensions are captured on `<img>` load for the Original preset. The embed node-view shares the resizer, so its handle icon is fixed too.

### ✨ Pinch-zoom + pan an image opened in its own tab
Opening an image attachment in a separate tab (double-click a chip) used to show a static fit-to-pane image. It now supports **pinch-to-zoom and pan**: trackpad pinch (ctrl+wheel), scroll / drag to pan, double-click to toggle fit ↔ 2×, and two-finger touch pinch on touchscreens — all cursor-anchored so the content under the pointer stays fixed, with clamped panning so a zoomed image can't be dragged off-screen. Header zoom buttons (zoom out / live percent / zoom in / reset) give precise control. A `use-image-pan-zoom` composable drives a CSS `translate`/`scale` transform on the `<img>` (fit via `object-contain` at scale 1); zoom state persists across tab switches (KeepAlive) and resets on each new image. `zoom-in` / `zoom-out` / `maximize-2` added to the curated icon registry. PDF / text / video / audio previews are unchanged.

### ✨ Open an image or attachment in a new tab or focus window from the right-click menu
Right-clicking an inline image or an attachment chip in the editor now offers **Open in new tab** (opens the preview as a tab in the current pane) and **Open in new window** (opens a focus-mode window with just that attachment — sidebar and notes list hidden). The existing corner-drag resize and chip double-click → split-right paths are unchanged. Detection reuses ProseMirror's `NodeSelection` (set on atom nodes on right-click) with a `posAtCoords` + `nodeAt`/`nodeAfter` fallback; `src`-only inline images (external / data URLs with no attachment blob) are excluded. The focus window reuses the pane-window machinery with a new optional `focus` flag threaded through the `openPaneWindow` tRPC procedure → main → URL (`?focus=1`), and `App.vue` enables focus mode on that param. `useEditorContextMenu` gained a per-pane `groupId` param so "new tab" lands in the right split. `contextMenu.openInNewTab` added (en/de); 3 new contract tests (17 pass); typecheck clean.

## [0.19.0] - 2026-08-01

### ✨ Reopen the last closed tab (browser-style)
Closing an editor tab and immediately regretting it used to mean finding the note again in the list. `Cmd/Ctrl+Shift+T` now reopens the most recently closed tab, like a browser — and it keeps reopening on each press, walking back through up to 25 closed tabs in LIFO order. The tab is recreated in the active pane and, for note tabs, its **in-tab back/forward history is restored** so Back/Forward still work after reopening; attachment and search-result tabs reopen too. A matching **Reopen Closed Tab** command lives in the command palette (disabled when there's nothing to reopen). The closed-tab stack is tracked in the editor-layout store (snapshotted in `closeTab`), capped at 25, and **cleared on an account/context switch** so a reopened tab never references another account's notes. Wired through a new `app:reopen-closed-tab` menubar IPC channel (preload → `App.vue`), following the existing `app:close-tab` pattern; `menu.reopenClosedTab` / `command.reopenClosedTab` added (en/de). 7 new contract tests; typecheck clean.

### ✨ Open a new window for the current account
`Cmd/Ctrl+Shift+N` opens a fresh full-shell app window bound to the **same account** as the window you ran it from, so you can have the same account open in two windows side-by-side (e.g. one on All Notes, one on a search). It reuses the existing account-switcher "Open in new window" path: the menubar item resolves the focused window's bound context and calls the main-side `openAccountWindow`, loading the renderer with `?ctx=<contextId>` so it boots that account's own encrypted SQLite context — falling back to the local context when no window is bound yet. A matching **New Window** entry is in the command palette (calling the `window.openAccountWindow` tRPC bridge with the current context). The File menu gained the item next to New Note; `menu.newWindow` / `command.newWindow` added (en/de).

### ✨ Toggle the sidebar & focus mode from the menubar
The sidebar collapse toggle and the distraction-free focus-mode toggle were reachable only from the command palette and the titlebar button — they now have dedicated **keyboard shortcuts in the menubar**: `Cmd/Ctrl+S` toggles the sidebar and `Cmd/Ctrl+.` toggles focus mode (which hides both the sidebar and the notes list). The View menu is now built explicitly (instead of the default `viewMenu` role) so the two toggles sit alongside the standard zoom / devtools / fullscreen items. The toggle state still lives in the renderer's shell store, so the menubar items signal it through a new `app:shell-toggle` IPC channel (preload → `App.vue`), the same shape as `app:close-tab`; the renderer ignores the signal before the shell is visible (login / Settings window). `menu.view` / `menu.toggleSidebar` / `menu.toggleFocusMode` added (en/de). (`Cmd+S` is safe to repurpose — notes autosave, so there was no Save binding to clobber.)

### ✨ Per-note semantic map in the right sidebar
The right-sidebar panel that already hosts the **Table of contents** and the **Minimap** gained a third view: a per-note **semantic map**. Where the full-screen vector visualizer graphs the whole account, this one zooms in on a single note's immediate neighbourhood — the note itself sits at the centre and its ~12 nearest semantic neighbours radiate out as spokes, each edge drawn thicker the more similar the note. It reuses the same vector pipeline (note centroids, the `vec_notes` KNN, PCA-2D projection, and the theme-aware canvas bridge) rather than new math: a new `getNoteNeighbourhood` helper loads the note's centroid + KNN-sorted neighbours with their real centroid↔centroid cosine similarity, and a new `NoteVisualizer.vue` canvas projects just that subset (translated so the centre is at the origin) and auto-fits it to the panel each render. Clicking a spoke — or a compact related-notes list under the canvas — opens that note in a **new editor tab** so exploring the neighbourhood doesn't replace the note you're visualizing. Per-tab like the other two views, so two split panes show two different notes' neighbourhoods; width matches ToC's (`w-80`); the graph re-renders live on light/dark theme switch. Gated by semantic search — it shows an "enable semantic search" empty state when the index is off, and an empty state when no live related notes are found. `TocMode` widened to `"toc" | "minimap" | "visualizer"`; `toc.visualizer` / `visualizerLoading` / `visualizerDisabled` / `visualizerEmpty` / `related` added (en/de).

### ✨ The Changelog shows every release since your version, not just the newest
The What's New / Changelog view (the standalone window **and** the in-app dialog) previously rendered only the single newest version's release notes — so a user on an older build saw just the latest section and nothing about the releases in between. It now shows **every release from your installed version up to the newest** (inclusive), newest-first, so you see the full set of changes you've missed. The raw `CHANGELOG.md` was already fetched from GitHub at runtime; a new `formatChangelogRange` helper in `utils/markdown.ts` slices all `## [x.y.z]` sections whose version is ≥ the installed `__APP_VERSION__` (skipping `## [Unreleased]`), falling back to all version sections when the installed version predates the recorded history. The in-app dialog gained its own on-mount remote fetch it previously lacked (it only had the baked text). Content precedence: remote range → the provider's single-version release notes → the baked installed-version section. `formatBundledChangelog` is retained for the test-changelog path. 7 contract tests added; 1971 pass.

### 🐛 The auto-updater now reports why an update failed (macOS groundwork)
On macOS, "Check for updates" could fail silently — no error, no update, no clue why — because the `electron-updater` error was swallowed: the event handler only reset the download progress bar and the `check()` catch reset progress without logging. The real message is now written to the main console **and** surfaced in the existing update-error banner (Updates settings section / changelog views) through a new optional `error` field on `UpdateStatus` that `applyStatus` mirrors into the store's `lastError`. This is diagnostic groundwork: the macOS build is unsigned/ad-hoc-signed (not Developer ID, not notarized), so the underlying failure is almost certainly a code-signature mismatch on the downloaded update or a Gatekeeper block on the quarantined replacement — but the exact layer can't be confirmed until the string is visible. The targeted fix (signing + notarization, or a quarantine-stripping workaround for the alpha) follows once the error is read from a packaged build. Windows updates are unaffected (the updater there only verifies the blockmap hash, not Authenticode).

### ✨ Highlighted text stays readable against any highlight color
Text highlighted with a color swatch could be unreadable — light theme text on a dark-blue/purple highlight, or dark-theme light text on a yellow/green highlight — because the highlight extension forced the text to keep the theme text color (`color: inherit`) regardless of the highlight background, and that inline style beat every stylesheet rule. The text color now **adapts per highlight swatch and per theme** to meet WCAG AA contrast against its highlight background, recomputing live when you switch themes (no need to re-apply). It reuses the same OKLCH/WCAG contrast engine as the block-colorize feature: `theme-vue`'s new `highlightToCSS` emits a `--hl-text-<name>` variable per preset swatch = the theme text color adjusted in lightness (hue preserved) until it clears 4.5:1 against the swatch. A custom `Highlight` extension drops the inline `color: inherit` so CSS can drive the text color, and `style.css` maps each preset swatch's `data-color` to its variable. The shared `adjustForContrast` now tries both lightness directions when the primary one can't reach the target (mid-luminance backgrounds like the gray/orange swatches pointed the wrong way) — strictly an improvement, block-colorize output is unchanged. Covers the seven preset swatches; custom OS-picker colors fall back to the previous behavior. 5 new contract tests; typecheck clean.
...
The proactive suggestions bar (notebook/tag/color chips + related notes) can now be **summoned manually** from the command palette — **Show note suggestions** — and the manual call is **not** restricted to the automatic criteria: it runs for the current note whether or not it already has a notebook/tag/color, or how long it is (it only shows nothing when the run genuinely finds nothing to suggest). Once open, the bar is **keyboard-navigable**: the first chip auto-focuses and ←/→ (and ↑/↓) cycle through every chip and action — notebook/tag/color chips, each related note's Open and Link buttons, and the dismiss button — with Home/End jumps and an accent focus ring; Enter/Space activates the focused chip, Escape dismisses. The keydown handler is scoped to the bar and only fires while a chip has focus, so it never fights editor typing (which keeps focus in the editor). Wired via a per-pane `suggestionsSignal` on the editor store (mirrors the existing `findSignal` "Find in note" path) + a `show()`/`forcedOpen` override on `use-note-suggestions`; `NoteSuggestions.vue` gained the keyboard nav + a `:keyboard-active` prop.

### 🐛 Suggestions bar is readable on the dark theme
The floating suggestion pill used the faint glass-surface tint (5% paragraph), which over editor content on a dark theme let the editor text show through and the chips lose contrast. It now sits on a near-opaque dark surface (the theme's `--background` at 92%) on any dark theme. Added a generic `data-color-scheme` marker in `theme-vue`'s injector (alongside the per-theme `data-theme`) so the override keys off "is this a dark theme" across built-in *and* custom themes — a hook the rest of the app can reuse.

### ✨ Daily-notes timeline scrolls endlessly + opens on today without a jump
The Daily Notes date timeline no longer has a hard ±60-day edge: scrolling near the top or bottom extends the window in that direction, with the scroll offset preserved when prepending past days so the view never jumps. Entering the view now centers today **instantly** (no smooth scroll animation from the top of the list down to today). The ↑/↓ page buttons were removed; a **Today** button in the header jumps back to today after scrolling far away. Wired in `DailyNotesTimeline.vue` (growable `windowStart`/`windowEnd`, passive `@scroll` extension, `behavior:"auto"` first-center, `goToToday`); `common.today` added (en/de).
